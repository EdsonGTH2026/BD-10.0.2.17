...
--pFMKDatosCobranzaSolicitada_ParaCliente 'PONCE CASTRO PAULA','UMC1809791',''
CREATE procedure [dbo].[pFMKDatosCobranzaSolicitada_ParaCliente] @nombrecompleto varchar(200)
, @CodUsuarioCobranza varchar(20),@ControlInterno varchar(20)
as

--declare @nombrecompleto varchar(200)
--declare @CodUsuarioCobranza varchar(20)
--declare @ControlInterno varchar(20)
--set @nombrecompleto='PONCE CASTRO PAULA'
--set @CodUsuarioCobranza='UMC1809791'
--set @ControlInterno=''

declare @CodUsuario varchar(20)
select top 1 @CodUsuario=codusuario from tcspadronclientes with(nolock) where nombrecompleto like '%'+@nombrecompleto+'%'
...
--declare @CodUsuarioCobranza varchar(20)
--set @CodUsuarioCobranza = 'SCC0912751'
...
+d.otroscargos+d.impuestos+d.cargomora deudatotal,c.nrocuotasporpagar, 0 Cuotasatrasadas,c.nrodiasatraso,
d.codusuario,
--isnull((select CodUsuarioCobranza from tClOficinas where CodOficina = c.CodOficina),'SIN USUARIO') as codusuario,
--u.usuario, (SE CAMBIO X LINEA DE ABAJO)
--isnull((select us.Usuario from tClOficinas as o inner join tsgusuarios as us on o.CodUsuarioCobranza = us.CodUsuario  where o.CodOficina = c.CodOficina),'SIN USUARIO') as usuario,
isnull((select top 1 Usuario from tsgusuarios where codusuario = @CodUsuarioCobranza), 'SIN USER') as usuario,
c.codasesor,
--(NUEVA LINEA PARA USUARIO COBRANZA)
--isnull((select us.Usuario from tClOficinas as o inner join tsgusuarios as us on o.CodUsuarioCobranza = us.CodUsuario  where o.CodOficina = c.CodOficina),'SIN USUARIO') as UsuCobranza
isnull((select top 1 Usuario from tsgusuarios where codusuario = @CodUsuarioCobranza), 'SIN USER') as UsuCobranza
...
where 
--c.fecha=@fecha and c.nrodiasatraso>60 and c.nrodiasatraso<161
d.codusuario = @CodUsuario
