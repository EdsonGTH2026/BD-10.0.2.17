...
GO
CREATE PROCEDURE [dbo].[pCsCaRptSituacionxOficinaConsol] @Fecha smalldatetime AS
...
insert into @tbAuxCartera (Fecha,CodPrestamo,CodUsuario,CodOficina,NomOficina,Estado,MontoDesembolso,NroDiasAtraso,OtrosCargos,SaldoCapital,SaldoINTE,SaldoINPE,SaldoINTEVIG,SaldoINPEVIG,SaldoINTESus,SaldoINPESus,NumReprog,CodPrestamoVigReg)
SELECT     cd.Fecha, cd.CodPrestamo, cd.CodUsuario, CAST(cd.CodOficina AS int) AS CodOficina, tClOficinas.NomOficina AS NomOficina, c.Estado, 
                      ...
                      cd.InteresCtaOrden AS SaldoINTESus, cd.MoratorioCtaOrden AS SaldoINPESus, ISNULL(c.NumReprog, 0) 
...
SELECT     Fecha, CodOficina, NomOficina, SUM(NroCliente) AS NroCliente, SUM(NroPtmos) AS NroPtmos, SUM(MontoDesembolso) AS MontoDesembolso, 
                      SUM(SaldoCapital) AS SaldoCapital, SUM(NroCliente_0Dias) AS NroCliente_0Dias, SUM(NroCliente_30Dias) AS NroCliente_30Dias, 
                      SUM(NroCliente_90Dias) AS NroCliente_90Dias, SUM(NroCliente_MasDias) AS NroCliente_MasDias, SUM(NroPtmos_0Dias) AS NroPtmos_0Dias, 
                      SUM(NroPtmos_30Dias) AS NroPtmos_30Dias, SUM(NroPtmos_90Dias) AS NroPtmos_90Dias, SUM(NroPtmos_MasDias) AS NroPtmos_MasDias, 
                      SUM(SaldoCap_0Dias) AS SaldoCap_0Dias, SUM(SaldoCap_30Dias) AS SaldoCap_30Dias, SUM(SaldoCap_90Dias) AS SaldoCap_90Dias, 
                      SUM(SaldoCap_MasDias) AS SaldoCap_MasDias
FROM         (SELECT     Fecha, CodOficina, NomOficina, NroCliente, NroPtmos, MontoDesembolso, SaldoCapital, NroCliente_0Dias, NroCliente_30Dias, 
                                              ...
                                              SaldoCap_MasDias
                       FROM          (SELECT     Fecha, CodOficina, NomOficina, COUNT(DISTINCT CodUsuario) AS NroCliente, COUNT(DISTINCT CodPrestamo) AS NroPtmos, 
                                                                      SUM(ISNULL(MontoDesembolso, 0)) AS MontoDesembolso, SUM(SaldoCapital) AS SaldoCapital, SUM(NroCliente_0Dias) 
                                                                      AS NroCliente_0Dias, SUM(NroCliente_30Dias) AS NroCliente_30Dias, SUM(NroCliente_90Dias) AS NroCliente_90Dias, 
                                                                      SUM(NroCliente_MasDias) AS NroCliente_MasDias, NroPtmos_0Dias, NroPtmos_30Dias, NroPtmos_90Dias, 
                                                                      NroPtmos_MasDias, SUM(SaldoCap_0Dias) AS SaldoCap_0Dias, SUM(SaldoCap_30Dias) AS SaldoCap_30Dias, 
                                                                      SUM(SaldoCap_90Dias) AS SaldoCap_90Dias, SUM(SaldoCap_MasDias) AS SaldoCap_MasDias
                                               FROM          (SELECT     Fecha, CodPrestamo, CodUsuario, CodOficina, NomOficina, Estado, NroDiasAtraso, 
                                                                                              ...
                                                                                              THEN (CASE a.NroDiasAtraso WHEN 0 THEN 1 ELSE 0 END) ELSE 0 END) END AS NroCliente_0Dias, 
                                                                                              ...
                                                                                              END AS NroCliente_30Dias, CASE a.NumReprog WHEN 0 THEN (CASE WHEN a.NroDiasAtraso > 30 AND 
												...
												ELSE  (  CASE WHEN NOT (CodPrestamoVigReg IS NULL) THEN 0 ELSE 1 END )   END
                                                                                              ...
                                                                                              END AS NroPtmos_30Dias, CASE a.NumReprog WHEN 0 THEN (CASE WHEN a.NroDiasAtraso > 30 AND 
						...
						CASE a.NumReprog WHEN 0 THEN (CASE WHEN a.NroDiasAtraso > 0 AND 
                                                                                              ...
                                                                                              a.NroDiasAtraso < 90 THEN a.saldocapital + isnull(a.SaldoINTEVIG, 0) + isnull(a.SaldoINPEVIG, 0) ELSE 0 END) 
                                                                                              ELSE (CASE WHEN NOT (CodPrestamoVigReg IS NULL) 
                                                                                              THEN (CASE WHEN a.NroDiasAtraso > 30 AND a.NroDiasAtraso < 90 THEN a.saldocapital + isnull(a.SaldoINTEVIG, 0) 
                                                                                              + isnull(a.SaldoINPEVIG, 0) ELSE 0 END) ELSE 0 END) END AS SaldoCap_90Dias, 
...
SaldoINPEVIG, SaldoINTESus, SaldoINPESus, NumReprog, CodPrestamoVigReg  FROM  @tbAuxCartera

) a) b
                                               GROUP BY Fecha, CodOficina, NomOficina, NroPtmos_0Dias, NroPtmos_30Dias, NroPtmos_90Dias, NroPtmos_MasDias) c) d
GROUP BY Fecha, CodOficina, NomOficina
...
FROM         (SELECT     Fecha, CodOficina, NomOficina, NroCliente, NroPtmos, MontoDesembolso, SaldoCapital, NroCliente_0Dias, NroCliente_30Dias, 
                       ...
                       FROM          (SELECT     Fecha, CodOficina, NomOficina, COUNT(DISTINCT CodUsuario) AS NroCliente, COUNT(DISTINCT CodPrestamo) AS NroPtmos, 
                                               ...
                                               FROM          (SELECT     Fecha, CodPrestamo, CodUsuario, CodOficina, NomOficina, Estado, NroDiasAtraso, 
                                                                                              ...
                                                                                              ELSE (CASE WHEN codprestamo IN
                                                                                                  (SELECT     codprestamo
                                                                                                    FROM          tCsRenegociadosVigentes) THEN (CASE NroDiasAtraso WHEN 0 THEN 1 ELSE 0 END) ELSE 0 END) 
                                                                                              END AS NroCliente_0Dias, CASE NumReprog WHEN 0 THEN (CASE WHEN NroDiasAtraso > 0 AND 
                                                                                              NroDiasAtraso <= 30 THEN 1 ELSE 0 END) ELSE (CASE WHEN codprestamo IN
                                                                                                  (SELECT     codprestamo
                                                                                                    FROM          tCsRenegociadosVigentes) THEN (CASE WHEN NroDiasAtraso > 0 AND 
                                                                                              NroDiasAtraso <= 30 THEN 1 ELSE 0 END) ELSE 0 END) END AS NroCliente_30Dias, 
                                                                                              CASE NumReprog WHEN 0 THEN (CASE WHEN NroDiasAtraso > 30 AND NroDiasAtraso < 90 THEN 1 ELSE 0 END) 
                                                                                              ELSE (CASE WHEN codprestamo IN
                                                                                                  (SELECT     codprestamo
                                                                                                    FROM          tCsRenegociadosVigentes) THEN (CASE WHEN NroDiasAtraso > 30 AND 
                                                                                              NroDiasAtraso < 90 THEN 1 ELSE 0 END) ELSE 0 END) END AS NroCliente_90Dias, 
                                                                                              ...
                                                                                              ELSE (CASE WHEN codprestamo IN
                                                                                                  (SELECT     codprestamo
                                                                                                    FROM          tCsRenegociadosVigentes) THEN (CASE NroDiasAtraso WHEN 0 THEN codprestamo ELSE '' END) 
                                                                                              ELSE '' END) END NroPtmos_0Dias, CASE NumReprog WHEN 0 THEN (CASE WHEN NroDiasAtraso > 0 AND 
                                                                                              NroDiasAtraso <= 30 THEN codprestamo ELSE '' END) ELSE (CASE WHEN codprestamo IN
                                                                                                  (SELECT     codprestamo
                                                                                                    FROM          tCsRenegociadosVigentes) THEN (CASE WHEN NroDiasAtraso > 0 AND 
                                                                                              NroDiasAtraso <= 30 THEN codprestamo ELSE '' END) ELSE '' END) END NroPtmos_30Dias, 
                                                                                              CASE NumReprog WHEN 0 THEN (CASE WHEN NroDiasAtraso > 30 AND 
                                                                                              NroDiasAtraso < 90 THEN codprestamo ELSE '' END) ELSE (CASE WHEN codprestamo IN
                                                                                                  (SELECT     codprestamo
                                                                                                    FROM          tCsRenegociadosVigentes) THEN (CASE WHEN NroDiasAtraso > 30 AND 
                                                                                              NroDiasAtraso < 90 THEN codprestamo ELSE '' END) ELSE '' END) END NroPtmos_90Dias, 
                                                                                              ...
                                                                                              ELSE 0 END) ELSE (CASE WHEN codprestamo IN
                                                                                                  (SELECT     codprestamo
                                                                                                    FROM          tCsRenegociadosVigentes) 
                                                                                              THEN (CASE NroDiasAtraso WHEN 0 THEN saldocapital + isnull(SaldoINTEVIG, 0) ELSE 0 END) ELSE 0 END) 
                                                                                              END AS SaldoCap_0Dias, CASE NumReprog WHEN 0 THEN (CASE WHEN NroDiasAtraso > 0 AND 
                                                                                              NroDiasAtraso <= 30 THEN saldocapital + isnull(SaldoINTEVIG, 0) + isnull(SaldoINPEVIG, 0) ELSE 0 END) 
                                                                                              ELSE (CASE WHEN codprestamo IN
                                                                                                  (SELECT     codprestamo
                                                                                                    FROM          tCsRenegociadosVigentes) THEN (CASE WHEN NroDiasAtraso > 0 AND 
                                                                                              NroDiasAtraso <= 30 THEN saldocapital + isnull(SaldoINTEVIG, 0) + isnull(SaldoINPEVIG, 0) ELSE 0 END) ELSE 0 END)
                                                                                               END AS SaldoCap_30Dias, CASE NumReprog WHEN 0 THEN (CASE WHEN NroDiasAtraso > 30 AND 
                                                                                              NroDiasAtraso < 90 THEN saldocapital + isnull(SaldoINTEVIG, 0) + isnull(SaldoINPEVIG, 0) ELSE 0 END) 
                                                                                              ELSE (CASE WHEN codprestamo IN
                                                                                                  (SELECT     codprestamo
                                                                                                    FROM          tCsRenegociadosVigentes) THEN (CASE WHEN NroDiasAtraso > 30 AND 
                                                                                              NroDiasAtraso < 90 THEN saldocapital + isnull(SaldoINTEVIG, 0) + isnull(SaldoINPEVIG, 0) ELSE 0 END) ELSE 0 END) 
                                                                                              END AS SaldoCap_90Dias, CASE WHEN NroDiasAtraso >= 90 THEN saldocapital + isnull(SaldoINTEVIG, 0) 
                                                                       ...
                                                                       FROM          (SELECT     cd.Fecha, cd.CodPrestamo, cd.CodUsuario, CAST(cd.CodOficina AS int) AS CodOficina, 
                                                                                                                      tClOficinas.NomOficina AS NomOficina, c.CodSolicitud, c.CodProducto, c.CodAsesor, 
                                                                                               ...
                                                                                               FROM          (SELECT     CodUsuario, Paterno + ' ' + Materno + ', ' + Nombres Cliente, CodDocIden, DI, 
                                                                                                                      ...
                                                                                                                      tClOficinas ON cd.CodOficina = tClOficinas.CodOficina
                                                                                               WHERE  (cd.Fecha = @Fecha) AND c.estado not in('CASTIGADO','ADMINISTRATIVO')) a) b
                                               GROUP BY Fecha, CodOficina, NomOficina, NroPtmos_0Dias, NroPtmos_30Dias, NroPtmos_90Dias, NroPtmos_MasDias) c) d
GROUP BY Fecha, CodOficina, NomOficina*/
GO