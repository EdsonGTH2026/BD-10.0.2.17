...
CREATE PROCEDURE [dbo].[pCsAhEstadoCuenta2] @Usuario VARCHAR(20), @Cuenta VARCHAR(25),@fraccioncta varchar(3),@renovado int, @PrimerCorte SMALLDATETIME, @UltimoCorte SMALLDATETIME    
AS    
set nocount on    
...
--set @Usuario=''    
--set @Cuenta='098-211-06-2-0-01576'--'098-112-06-2-9-04781'--'098-105-06-2-7-00192'    
--set @fraccioncta='0'    
--set @renovado=4  
--Declare @UltimoCorte SmallDateTime    
--Declare @PrimerCorte SmallDateTime    
--set @PrimerCorte='20231106 00:00:00'    
--set @UltimoCorte='20231110 00:00:00'    
...
FROM tCsAhorros with(nolock)    
WHERE codcuenta=@Cuenta and FraccionCta=@FraccionCta and Renovado=@Renovado AND Fecha = @fechaMax--@UltimoCorte2    
...
select    
 ...
 --select *    
  ...
  WHERE codcuenta=@Cuenta and FraccionCta=@FraccionCta and Renovado=@Renovado    
     ...
     (    
         ...
         FROM tcsahorros WITH(NOLOCK)    
   WHERE codcuenta=@Cuenta and FraccionCta=@FraccionCta and Renovado=@Renovado    
         AND Fecha>= @PrimerCorte2 AND Fecha<=@fechaMax--@UltimoCorte2    
     );    
    ...
    ,CASE    
       ...
       ELSE 0    
         ...
         WHERE t.Fecha >= @PrimerCorte2    
              ...
              AND (t.CodSistema = 'AH')    
     and t.codigocuenta=@Cuenta and t.FraccionCta=@FraccionCta and t.Renovado=@Renovado    
     ) AS x;    
 ...
 WHERE a.codcuenta=@Cuenta and a.FraccionCta=@FraccionCta and a.Renovado=@Renovado    
 and a.fecha=@fechaMax--@UltimoCorte2    
GO