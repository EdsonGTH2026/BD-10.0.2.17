...
GO
/*Reporte CARTA REGIONAL*/   
  
 ---OPTIMIZA SP 2023.10.16 ZCCU  
 --se ajustan consultas para reconocer las nuevas regiones 2024.05.10 ZCCU
CREATE procedure [dbo].[pCsCaFNCartaRegional]     
...
declare @baseKpi table(fecha smalldatetime,Region varchar(30),zona varchar(4),saldo0a30Ini money    
...
insert into @baseKpi    
select fecha,Region,z.zona,saldo0a30Ini,saldo31a89Ini,saldo90Ini,saldocapIni,saldo0a30Fin,saldo31a89Fin,saldo90Fin     
...
,nuevosPtmos,RenovPtmos,RAnticipaPtmos,ReactivaPtmos,totaPtmos     
,promRenovColoca,promRAnticipaColoca,promReactivaColoca,promNuevoColoca,promTotalColoca    
,porRenov,porRAnticipa,porReactiva,porNuevo,porTotal     
...
from FNMGConsolidado.dbo.tCaReporteKPI m  --consulta a la tabla kpi    
left outer join tclzona z on m.Region=z.nombre    
where  nomoficina='zz' and fecha=@fecha     
    
/*------CRECIMIENTO MENSUAL*/    
declare @crecimientoMes table (region varchar(30) --Variacion de cartera    
     ,zona varchar(4)    
...
declare @estadoAlcance table (region varchar(30),zona varchar(3),metacrecimiento money,porAlcance money,estadoAlcance varchar(8))    
insert into @estadoAlcance    
select m.region,m.zona,metacrecimiento     
...
from @baseKpi m    
left outer join @crecimientoMes v  on v.zona=m.zona     
    
/*--- IMOR INICIAL*/    
    
declare @imorIni table (  zona varchar(35)     
        ,Imor16ini money    
        ,Imor30ini money    
        ,Imor90ini money)    
insert into @imorIni          
--select     
--z.zona as 'zona',Imor16,Imor30,Imor90    
--from FNMGConsolidado.dbo.tCaReporteKPI m with(nolock)    
--left outer join tclzona z on m.Region=z.nombre    
--where fecha=@fecante     
--and nomoficina='zz'    
...
,(sum(case when c.nrodiasatraso>=30 then d.saldocapital else 0 end)  
 ...
 ,(sum(case when c.nrodiasatraso>=90 then d.saldocapital else 0 end)  
...
where c.fecha=@fecante---'20240509'
and  cartera='ACTIVA' and c.codprestamo not in(select codprestamo from tCsCarteraAlta with(nolock))
and c.codprestamo not in('97','231','230','999')
group by Z.ZONA


/*-----IMOR FINAL */    
declare @imor table(zona varchar(3)    
      ,region varchar(30)    
      ,Imor16Ini money    
      ,Imor30Ini money    
      ,Imor90Ini money    
...
insert into @imor         
select i.zona,b.region,Imor16Ini,Imor30Ini,Imor90Ini    
...
from @imorIni i    
left outer join @baseKpi b on i.zona=b.zona    
...
-----COBRANZA PUNTUAL X REGION     
declare @pDiariaxRegion table(region varchar(30),puntualc1 money,puntualC2 money,puntualC3_4 money    
...
,sum(case when rangoCiclo in ('c.3-4')  then (monto_puntual+monto_anticipado) else 0 end) puntualC3_5    
,sum(case when rangoCiclo in ('c.5+')  then (monto_puntual+monto_anticipado) else 0 end) puntualC6m    
...
declare @CobranzaPuntual table(fecha smalldatetime ,region varchar(30)    
...
declare @previo table(fecha smalldatetime,region varchar(30),zona varchar(3)    
...
insert into @previo    
select     
b.fecha,b.region,b.zona,saldo0a30Ini,saldo31a89Ini,saldo90Ini,saldocapIni    
...
from @baseKpi b    
left outer join @imor i on i.zona=b.zona    
left outer join @estadoAlcance a on i.zona=a.zona    
left outer join @crecimientoMes m on b.zona=m.zona    
left outer join @CobranzaPuntual p on p.region=b.region    
...
declare @Cubetas table (zona varchar(4),sal1a7ini money,sal8a15ini money,sal16a30ini money,sal31ini money    
,salTotalini money,ptmos1a7ini int,ptmos8a15ini int,ptmos16a30ini int,ptmos31ini int,ptmosTotalini int,sal1a7fin money,sal8a15fin money    
,sal16a30fin money,sal31fin money,salTotalfin money,ptmos1a7fin int,ptmos8a15fin int,ptmos16a30fin int,ptmos31fin int,ptmosTotalfin int    
,varSaldo1a7 money,varSaldo8a15 money,varSaldo16a30 money,varSaldo31 money,varSaldoTotal money,varPtmos1a7 money,varPtmos8a15 money    
...
from @previo p    
left outer join @CubetasxSuc i on i.zona=p.zona    
...
select a.zona,a.codoficina    
 ...
 ,case when c.cartera<>'CASTIGADA' then  case when c.NroDiasAtraso>=16 then  cd.saldocapital  else 0 end else 0 end D16saldo    
  ...
  ,case when c.cartera<>'CASTIGADA' then  case when c.NroDiasAtraso>=0 then cd.saldocapital  else 0 end else 0 end D0saldo    
   ...
   group by a.zona,a.codoficina,cosecha    
   order by zona,codoficina,cosecha    
...
delete  FNMGConsolidado.dbo.tcaCartaRegional where fecha=@fecha    
insert into FNMGConsolidado.dbo.tcaCartaRegional    
    
select     
@fecha fecha,@diacorte DiaCorte,p.region,substring(p.zona,2,3) Nrozona,p.zona zona    
,saldo0a30Ini,saldo31a89Ini,saldo90Ini,saldocapIni    
,saldo0a30Fin,saldo31a89Fin,saldo90Fin,saldocapFinal,crecimiento0a30,crecimiento31a89,crecimiento90,crecimientoTotal    
,isnull(metacrecimiento,0)metacrecimiento,isnull(porAlcance,0)porAlcance    
,case when isnull(metacrecimiento,0)=0 and isnull(porAlcance,0)=0 then '' else estadoAlcance end estadoAlcance    
--referente de colocacion       
,isnull(montoColocacion,0)montoColocacion,isnull(porColocacion,0)porColocacion         --campos nuevos    
,Imor16Ini,Imor30Ini,Imor90Ini,Imor16,Imor30,Imor90,descripImor16,descripImor30,descripImor90    
--clientes      
...
--Cobranza Puntual    
,CobranzaPuntalC1,CobranzaPuntualC2,CobranzaPuntualC3_4 CobranzaPuntualC3a4,CobranzaPuntualC5m ,CobranzaPuntalTotal     
--colocacion    
...
from @previo p     
inner join @det d on d.zona=p.zona    
inner  join @Cubetas c on p.zona=c.zona    
where p.zona<>'ZPE'    
GO