...
GO
CREATE PROCEDURE [dbo].[pCsCaSaldosClientesReprestamosxProdxAse] @Fecha smalldatetime AS
SELECT     Fecha, Tecnologia, CodOficina, NomOficina, NombreProd, ASESOR, SUM(NroCliente) AS NroCliente, SUM(NroPtmos) AS NroPtmos, 
                      SUM(MontoDesembolso) AS MontoDesembolso, SUM(SaldoCapital) AS SaldoCartera, SUM(NroCliente_0Dias) AS NroCliente_0Dias, 
                      SUM(NroCliente_30Dias) AS NroCliente_30Dias, SUM(NroCliente_90Dias) AS NroCliente_90Dias, SUM(NroCliente_MasDias) AS NroCliente_MasDias, 
                      SUM(NroPtmos_0Dias) AS NroPtmos_0Dias, SUM(NroPtmos_30Dias) AS NroPtmos_30Dias, SUM(NroPtmos_90Dias) AS NroPtmos_90Dias, 
                      SUM(NroPtmos_MasDias) AS NroPtmos_MasDias, SUM(SaldoCap_0Dias) AS SaldoCap_0Dias, SUM(SaldoCap_30Dias) AS SaldoCap_30Dias, 
                      SUM(SaldoCap_90Dias) AS SaldoCap_90Dias, SUM(SaldoCap_MasDias) AS SaldoCap_MasDias, SUM(NroCliente_Nuevos) AS NroCliente_Nuevos, 
                      SUM(NroCliente_Repres) AS NroCliente_Repres, SUM(Desem_Nuevos) AS Desem_Nuevos, SUM(Desem_Repres) AS Desem_Repres, SUM(Capital) 
                      AS Capital, SUM(SaldoINTE) AS SaldoINTE, SUM(SaldoINPE) AS SaldoINpE
FROM         (SELECT     Fecha, Tecnologia, CodOficina, NomOficina, NombreProd, NroCliente, NroPtmos, MontoDesembolso, SaldoCapital, NroCliente_0Dias, 
                                              NroCliente_30Dias, NroCliente_90Dias, NroCliente_MasDias, CASE WHEN NroPtmos_0Dias = '' THEN 0 ELSE 1 END AS NroPtmos_0Dias, 
                                              ...
                                              SaldoINPE
                       FROM          (SELECT     Fecha, Tecnologia, CodOficina, NomOficina, NombreProd, COUNT(DISTINCT CodUsuario) AS NroCliente, 
                                                                      COUNT(DISTINCT CodPrestamo) AS NroPtmos, SUM(ISNULL(MontoDesembolso, 0)) AS MontoDesembolso, SUM(SaldoCapital) 
                                                                      AS SaldoCapital, SUM(NroCliente_0Dias) AS NroCliente_0Dias, SUM(NroCliente_30Dias) AS NroCliente_30Dias, 
                                                                      SUM(NroCliente_90Dias) AS NroCliente_90Dias, SUM(NroCliente_MasDias) AS NroCliente_MasDias, NroPtmos_0Dias, 
                                                                      NroPtmos_30Dias, NroPtmos_90Dias, NroPtmos_MasDias, SUM(SaldoCap_0Dias) AS SaldoCap_0Dias, SUM(SaldoCap_30Dias) 
                                                                      ...
                                                                      SUM(SaldoINPEVIG) SaldoINpE
                                               FROM          (SELECT     Fecha, CodPrestamo, CodUsuario, CodOficina, NomOficina, NombreProd, Estado, NroDiasAtraso, SaldoCapital Capital,
                                                                       ...
                                                                       FROM          (SELECT     cd.Fecha, cd.CodPrestamo, cd.CodUsuario, CAST(cd.CodOficina AS int) AS CodOficina, 
                                                                                                                      ...
                                                                                                                      tCsPadronClientes.Paterno + ' ' + tCsPadronClientes.Materno + ', ' + tCsPadronClientes.Nombres AS Asesor,
                                                                                                                       tCsPadronCarteraDet.SecuenciaCliente, tCaProducto.NombreProd
                                                                                               FROM          tCsCartera c INNER JOIN
                                                                                                                      ...
                                                                                                                      cd.CodUsuario = tCsPadronCarteraDet.CodUsuario INNER JOIN
                                                                                                                      tCaProducto ON c.CodProducto = tCaProducto.CodProducto LEFT OUTER JOIN
                                                                                               ...
                                                                                               WHERE      (cd.Fecha = @Fecha) AND (c.Estado <> 'CASTIGADO')) a) b
                                               GROUP BY Fecha, Tecnologia, CodOficina, NomOficina, NombreProd, NroPtmos_0Dias, NroPtmos_30Dias, NroPtmos_90Dias, 
                                                                      NroPtmos_MasDias, ASESOR) c) d
GROUP BY Fecha, Tecnologia, CodOficina, NomOficina, NombreProd, ASESOR
