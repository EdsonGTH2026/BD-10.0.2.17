...
----exec pCsCuadroCASucursalMesProgresemos '20170828'
--drop procedure pCsCuadroCASucursalMesProgresemos
CREATE procedure [dbo].[pCsCuadroCASucursalMesProgresemos] @fecfin datetime
as
...
inner join (
	...
	,sum(case when c.codfondo='20' then cd.saldocapital*0.7 else 0 end) saldocapital
	,sum(case when c.codfondo='20' then (cd.interesvigente+cd.interesvencido)*0.7 else 0 end) interes
...
inner join (
	...
	,sum(case when c.codfondo='20' then cd.saldocapital*0.7 else 0 end) saldocapital
	,sum(case when c.codfondo='20' then (cd.interesvigente+cd.interesvencido)*0.7 else 0 end) interes
	,sum(case when c.codfondo='20' then cd.interesctaorden*0.7 else 0 end) ctaorden
	,sum(cd.moratoriovigente) + sum(cd.moratoriovencido) moratorio
	FROM tCsCartera c with(nolock) 
	inner join tcscarteradet cd with(nolock) on c.fecha=cd.fecha and c.codprestamo=cd.codprestamo  
...
inner join (
	...
	,sum(case when c.codfondo='20' then t.montocapitaltran*0.7 else 0 end) capital
	--,sum(montointerestran) interes
	,sum(case when c.codfondo='20' then t.montointerestran*0.7 else 0 end) interes
...
inner join (
	...
	,sum(case when c.codfondo='20' then pc.monto*0.7 else 0 end) monto
...
inner join (
	...
	,sum(case when c.codfondo='20' then d.interesdevengado*0.7 else 0 end)  interesdevengado
...
GRANT EXECUTE ON [dbo].[pCsCuadroCASucursalMesProgresemos] TO [marista]
GO