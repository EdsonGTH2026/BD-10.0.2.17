...
GO
--SP_HELPTEXT colocacionmiiodos1
CREATE procedure [dbo].[colocacionmiiodos1] as   
select  dbo.fdufechaaperiodo(p.desembolso) periodo    
,sum(p.monto) as 'colocacion'    
,count(p.codprestamo) as 'nro'    
,sum(case when p.monto>900 and ciclo>=5 then p.monto else 0 end ) as 'colocacionop1'    
,sum(case when p.monto>900 and ciclo>=5 then 1 else 0 end ) as 'nroop1'    
into #grafanamiio    
...
,sum(case when  t.NroDiasAtraso >= 3 and monto>900 and d.ciclo>=5 then isnull(t.SaldoCapital,0) else 0 end ) as 'cermay3op1'    
,sum(case when  t.NroDiasAtraso >= 3 and monto>900 and d.ciclo>=5 then 1 else 0 end ) as 'nrocermay3op1'    
,sum(case when  t.NroDiasAtraso < 3 and monto>900 and d.ciclo>=5 then isnull(t.SaldoCapital,0) else 0 end ) as 'cer3op1'    
