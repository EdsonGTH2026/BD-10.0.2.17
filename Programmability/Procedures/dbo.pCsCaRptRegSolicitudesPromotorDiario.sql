...
GO
CREATE procedure [dbo].[pCsCaRptRegSolicitudesPromotorDiario] @codoficina varchar(100)
as
set nocount on
...
from (
	...
	from @fechas f
	left outer join tCaASolicitudes s with(nolock) on s.fechasolicitud=f.fecha and s.codoficina in(select codigo from @sucursales)
