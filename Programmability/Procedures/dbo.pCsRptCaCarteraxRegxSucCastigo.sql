...
SET ANSI_NULLS OFF
GO
CREATE procedure [dbo].[pCsRptCaCarteraxRegxSucCastigo] @fecha smalldatetime
as
...
,cd.codusuario,c.CodPrestamo,c.estado
...
,case when c.NroDiasAtraso>=90 then cd.codusuario else null end Dm90nroclie
,case when c.NroDiasAtraso>=90 then cd.codprestamo else null end Dm90nroptmo
,case when c.NroDiasAtraso>=90 then cd.saldocapital + cd.interesvigente+cd.interesvencido+cd.moratoriovigente+cd.moratoriovencido
  else 0 end Dm90saldo
,z.nombre region
...
where c.fecha=@fecha and c.cartera='CASTIGADA'
) a
group by fecha,tipo,sucursal,region,vigilancia
GO