...
CREATE procedure [dbo].[pCsCaIncOrgP1x] @fecha smalldatetime
as
set nocount on
...
,case	when categoria='MASTER' and PorDeuDese>=75 then 8.5
		...
		when categoria='JUNIOR A' and PorDeuDese>=75 then 5.5
		when categoria='JUNIOR A' and PorDeuDese>=65 and PorDeuDese<75 then 5
		when categoria='JUNIOR B' then 4.5
		else 4.5 end PorBono		
from (
	select codoficina,codasesor,coordinador,saldocapital,desembolso,pordeudese
	,case	when saldocapital<400000 then 'JUNIOR B'
			when saldocapital>=400000 AND saldocapital<750000 then 'JUNIOR A'
