...
GO
--drop procedure pCsABonoCarteraV2Sol
CREATE procedure [dbo].[pCsABonoCarteraV3Sol]	@fecha SMALLDATETIME
as
...
from (
	...
	from (
		...
		,case when nrenovaP>=1 
					then (case when cancelacionanterior>=@FechaIni1 and cancelacionanterior<=@fec_1ra then 0 else 1 end) 
		...
		from (
			...
			,count(distinct(case when fechadesembolso>=@FechaIni1 and fechadesembolso<=@fec_1ra and secuenciacliente<>1 
											then codusuario else null end)) nrenovaP
			--,sum(asignado) nasignado
			,count(distinct(asignado)) nasignado
			from (
				select c.fecha, c.codprestamo, cd.codusuario,c.codasesor,c.fechadesembolso
				,p.cancelacionanterior,p.secuenciacliente
				--,max(p.cancelacionanterior) cancelacionanterior,max(p.secuenciacliente) secuenciacliente
				--,count(distinct(
				...
				--and c.codasesor='HJB0908791'
				group by c.fecha, c.codprestamo, cd.codusuario, c.codasesor,c.fechadesembolso
				,p.cancelacionanterior,p.secuenciacliente,can.codasesor
			) x
...
,case when p.porrenovar=0 then 0 else cast((cast(p.nrenovadoM as decimal(10,2))/cast(p.porrenovar as decimal(10,2)))*100 as decimal(10,2)) end PorRenoClie
...
,isnull(MCN.valorprog,0)+isnull(MCR.valorprog,0) MCT
...
,case when (isnull(MCN.valorprog,0)+isnull(MCR.valorprog,0))=0 then 100-- si es Cero (0) significa que no tienen metas y se colocara el 100%
				else cast(((case when p.ncliactu-p.ncliante<0 then 0 else p.ncliactu-p.ncliante end)/(isnull(MCN.valorprog,0)+isnull(MCR.valorprog,0)))*100 as decimal(16,2)) end '%MNC'
,case when isnull(MCD.valorprog,0)=0 then 100-- si es Cero (0) significa que no tienen metas y se colocara el 100%
				else cast((p.montodesembolso/(isnull(MCD.valorprog,0)))*100 as decimal(16,2)) end '%MDE'
--,CEILING(isnull(MCN.valorprog,0)/2) MetaProgN
--,CEILING(isnull(MCR.valorprog,0)/2) MetaProgR
,isnull(MCN.valorprog,0) MetaProgN
,isnull(MCR.valorprog,0) MetaProgR
,saldomayor1dia
into #tmpca2
from #tmpca p
...
LEFT OUTER JOIN tCsCaFactoresCalcBono f1 ON (case when p.ncliactu-p.ncliante-p.nreasignado<0 then 0 else p.ncliactu-p.ncliante-p.nreasignado end) between f1.MtoMin and f1.MtoMax and f1.tipo = 24
--LEFT OUTER JOIN tCsCaFactoresCalcBono f1 ON (case when p.ncliactu-p.ncliante<0 then 0 else p.ncliactu-p.ncliante end) between f1.MtoMin and f1.MtoMax and f1.tipo = 24
--LEFT OUTER JOIN tCsCaFactoresCalcBono f2 ON (case when p.porrenovar=0 then 0 else cast((cast((p.nrenovadoM+p.nrenovadoFP) as decimal(10,2))/cast(p.porrenovar as decimal(10,2)))*100 as decimal(10,2)) end)
--																						between f2.MtoMin and f2.MtoMax and f2.tipo = 21
LEFT OUTER JOIN tCsCaFactoresCalcBono f22 ON p.moraactual between f22.MtoMin and f22.MtoMax and f22.tipo = 25
...
LEFT OUTER JOIN tCsCaFactoresCalcBono f2 ON (case when isnull(MCR.valorprog,0)=0 then case when cast((p.nrenovadoM+p.nrenovadoFP) as decimal(10,2))=0 then 0 else 100 end
																									else cast((cast((p.nrenovadoM+p.nrenovadoFP) as decimal(10,2))/isnull(MCR.valorprog,0))*100 as decimal(10,2)) 
																						 end)
																						between f2.MtoMin and f2.MtoMax and f2.tipo = 21
--Nuevos
...
where fecha=@fecha and tipocalculo=2

insert into tCsRptBonoCarteraV2
select b.Fecha,2 Tipo,b.codoficina,b.nomoficina,b.region,b.codasesor,b.nomasesor,b.puesto,b.estadoasesor,e.Ingreso,e.CodEmpleado
