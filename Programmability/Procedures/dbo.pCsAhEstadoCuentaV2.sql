...
--sp_helptext pCsAhEstadoCuenta  
 ...
 --GDF-20250930: Se genera versión 2 del SP para apuntar a la tabla tCsAhorrosBak en lugar de tCsAhorros
...
CREATE PROCEDURE [dbo].[pCsAhEstadoCuentaV2] @Usuario VARCHAR(20), @Cuenta VARCHAR(25),@fraccioncta varchar(3),@renovado int, @PrimerCorte SMALLDATETIME, @UltimoCorte SMALLDATETIME    
AS    
...
--set @Usuario='curbiza'    
--set @Cuenta='098-105-06-2-2-00243'  
--set @fraccioncta='0'    
--set @renovado=0    
  
-------- se agrega definicion de variables de fecha zccu 2024.01.18  
Declare @UltimoCorte2 SmallDateTime    
Declare @PrimerCorte2 SmallDateTime    
set @PrimerCorte2=@PrimerCorte --'20250101'--
set @UltimoCorte2=@UltimoCorte --'20250131'--
    ...
    BEGIN    
        IF @fechaMax <= @UltimoCorte2   --Si la fechaMax en menor igual a la fecha corte    
            BEGIN    
                ...
                FROM tCsAhorrosBak WITH(NOLOCK)    
                WHERE codcuenta=@Cuenta and FraccionCta=@FraccionCta and Renovado=@Renovado    
        END;    
            ELSE    
            BEGIN    
...
-- HLL    
   ...
   SELECT @ImpuestosPeriodo = ISNULL(SUM(MontoTotalTran), 0)    
     ...
     WHERE    Fecha >= @PrimerCorte2    
          ...
          AND TipoTransacNivel1 = 'E'    
          AND TipoTransacNivel3 IN(62)       
          AND extornado = 0; ---- zccu 2025.01.13 
...
@ImpuestosPeriodo AS Impuestos    
 ...
 ---select a.*    
     FROM tCsAhorrosBak a WITH(NOLOCK)    
