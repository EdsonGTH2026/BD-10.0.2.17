...
CREATE procedure [dbo].[pXaCAaRenovarVs2PRUEBA] @codoficina varchar(4),@codasesor varchar(15)
as
set nocount on
--declare @codoficina varchar(3)
--set @codoficina=325--'6'
--declare @codasesor varchar(15)
----set @codasesor='LFA801001F17I1'--'GAA1701981'--'CGM891025M5RR3'--'AGJ1104961'--
...
SELECT MAX(l.secuenciaCliente) AS ciclo ,e.codusuario, l.codPromotor
INTO #excepciones
FROM [10.0.2.14].finmas.dbo.tCaPoliticasExcepciones e
INNER JOIN tCsACaLIQUI_RR l
	ON l.codUsuario = e.codUsuario
WHERE convert(VARCHAR(8), e.fechaLimiteActualizacion, 112) = convert(VARCHAR(8), @fecha, 112)
GROUP BY e.codUsuario, l.codPromotor
...
where p.estado not in(9,10,11,70,71) and s.codoficina=@codoficina and s.codestado<>'ANULADO'
