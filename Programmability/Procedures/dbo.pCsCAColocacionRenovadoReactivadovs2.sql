...
GO
CREATE procedure [dbo].[pCsCAColocacionRenovadoReactivadovs2] @fecini smalldatetime,@fecfin smalldatetime
as
set nocount on
...
--and substring(lq.codprestamo,5,3)=(case when substring(p.codprestamo,5,3) ='370' then '370' else '170' end)--substring(lq.codprestamo,5,3)
...
,sum(case when dbo.fdufechaaperiodo(p.desembolso)=dbo.fdufechaaperiodo(l.cancelacion) 
	then --p.monto 
		cast(
		(case when p.primerasesor<>p.ultimoasesor and @fecfin>=l.s2inicio then p.monto
		else 0 end) 
		as decimal(16,2))
