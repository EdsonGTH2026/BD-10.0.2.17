...
GO
CREATE PROCEDURE [dbo].[pvINTFCuentaVr14] @fecha smalldatetime 
AS
SET NOCOUNT ON

--09-06-2020, Christofer: optimizado por Christofer
...
create table #CA(codprestamo char(19),tipo varchar(20),codusuario varchar(20))
insert into #CA
select codprestamo,tipo,codusuario
from tCsBuroxTblReINomVr14 with(nolock)
--drop table #CA
create table #Ca_unique(codprestamo char(19))
insert into #Ca_unique
select distinct codprestamo from #CA with(nolock)

--select codprestamo,count(codprestamo) nro
--from #CA with(nolock)
--group by codprestamo
--having count(codprestamo)>1

--select * from #Ca_unique with(nolock)
--where codprestamo in (select codprestamo from tCsCarteraAlta with(nolock))
--where codprestamo in (select codprestamo from tCsBuroDepuLey with(nolock))
--where codprestamo in (select codprestamo from tCaCtasLiqPago with(nolock))
...
FROM (
    ...
    FROM (
        ...
        WHERE (Estado='ACTIVO') and fecha=@fecha--'20200531'--
...
from(
	...
	--from tCsBuroxTblReINomVr14 b with(nolock) --6932
	...
	--from tCsBuroxTblReINomVr14 b with(nolock) --662
...
--00:00:07
SELECT t.Fecha,t.CodigoCuenta codprestamo, sum(t.MontoCapitalTran+t.MontoInteresTran+t.MontoINPETran) monto--> OJO: deberia ser lo que pago el cliente el dinero que dejo
...
--00:00:03
select distinct CodPrestamo, cast((datediff(d, c.FechaDesembolso , c.FechaVencimiento) / 30.4) as decimal(18, 2) ) as m2,convert(int,c.MontoDesembolso)
from tcscartera c with(nolock) 
where c.fecha=@fecha--'20200531'--
and c.codprestamo in(select codprestamo from #Ca_unique with(nolock))
union
select distinct c.CodPrestamo, cast((datediff(d, c.Desembolso , cc.FechaVencimiento) / 30.4) as decimal(18, 2) ) as m2,convert(int,cc.MontoDesembolso)
from tcspadroncarteradet c with(nolock)
...
-->>>>>>>>>>>>>> OSC
--<<<<<<<<<<<<<<<<<<<<<<<<<< --OSC 05-10-2019
select fecha,codprestamo,montodesembolso,fechadesembolso,NroCuotas,SaldoCapital,SaldoInteresCorriente,SaldoINVE,SaldoINPE,FechaUltimoMovimiento,NrodiasAtraso
,codproducto,ModalidadPlazo,CodMoneda,Judicial,Cartera,codoficina,CuotaActual,NroCuotasPagadas 
...
--00:00:10
select codprestamo--,seccuota,fechavencimiento
,sum(montocuota) saldo
from tcsplancuotas with(nolock)
where fecha=@fecha--'20200531' --
and numeroplan=0 and codprestamo in (select codprestamo from #Ca_unique with(nolock))
and estadocuota<>'CANCELADO'
and fechavencimiento<=@fecha--'20200531'--
group by codprestamo--,seccuota,fechavencimiento
...
from (
	...
	from(
		...
		from (
			...
			where numeroplan=0 and codprestamo in (select codprestamo from #Ca_unique with(nolock))
			and estadocuota<>'CANCELADO'
...
insert into tCsBuroxTblReICueVr14  --DESCOMENTAR
...
,MontoPagar  = case when (case when MontoPagar.MontoPagar > ROUND(c.SaldoCapital + c.SaldoInteresCorriente + c.SaldoINVE + c.SaldoINPE + 0.5, 0)
					then ROUND(c.SaldoCapital + c.SaldoInteresCorriente + c.SaldoINVE + c.SaldoINPE, 0)
					else MontoPagar.MontoPagar end)=0 then
...
,SaldoVencido    = case when c.codoficina='97' --then 0
                              then case when c.codprestamo in ('097-303-06-02-00580', '097-303-06-04-00463',
		                                                                '097-303-06-04-00544', '097-303-06-06-00528',
		                                                                '097-303-06-08-00609', '097-303-06-08-00744',
		                                                                '097-303-06-09-00628')
		                                then Vencido.SaldoVencido else 0 
                                   end
                              else CASE When c.NrodiasAtraso > 0 THEN Vencido.SaldoVencido ELSE 0 END 
                         end
...
inner join #CA v--(select * from tCsBuroxTblReINomVr14 with(nolock) where tipo='Cartera') v  ON v.Fecha=c.Fecha AND v.CodPrestamo=c.CodPrestamo 
ON v.CodPrestamo=c.CodPrestamo and v.tipo='Cartera'
...
--left outer join #MoProxAmorAtraso MontoPagarAtraso with(nolock) ON MontoPagarAtraso.CodPrestamo=c.CodPrestamo
...
) Avaluo ON --v.Fecha = Avaluo.Fecha AND 
v.CodPrestamo = Avaluo.Codigo
...
CROSS JOIN [FinAmigoExterno_191115].dbo.vINTFCabeceraVr14 vINTFCabecera with(nolock)
--where c.codprestamo='002-123-06-00-00185'
...
left outer join #MoProxAmor MontoPagar with(nolock) ON MontoPagar.CodPrestamo=c.CodPrestamo
--left outer join #MoProxAmorAtraso MontoPagarAtraso ON MontoPagarAtraso.CodPrestamo=c.CodPrestamo
LEFT outer JOIN (
        SELECT CodPrestamo, Round(SUM( MontoDevengado- MontoPagado - MontoCondonado) + 0.5, 0) AS SaldoVencido
        from #tCsMesPlanCuotas with(nolock)
        WHERE DiasAtrCuota = 1
...
,UltimoPago = dbo.fduFechaATexto(pd.cancelacion, 'DDMMAAAA')--dbo.fduFechaATexto(tCsCartera_1.FechaUltimoMovimiento, 'DDMMAAAA')
...
,SaldoActual = 0 
,'' AS LimiteCredito
,SaldoVencido=0--CASE When #tCsCarteraFecha.NrodiasAtraso > 0 THEN Vencido.SaldoVencido ELSE 0 END AS SaldoVencido
,PagosVencidos= 0
----MOP: Manner Of Payment
,'01' AS MOP
, '' AS HistoricoPagos
...
----select getdate(), '10' --borrar
--update tcsburoxtblreicuevr14 
