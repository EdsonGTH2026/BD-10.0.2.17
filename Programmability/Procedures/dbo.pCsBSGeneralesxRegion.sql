...
GO
CREATE PROCEDURE [dbo].[pCsBSGeneralesxRegion] AS
...
SELECT item, Oficina, Saldo, dbo.fduBSIndRes(4,@ultdia,item,2,Saldo) SaldoRes, Saldo0dias, Saldo90dias, 
Mora90dias, dbo.fduBSIndRes(4,@ultdia,item,6,Mora90dias) M90Res, Mora0dias, dbo.fduBSIndRes(4,@ultdia,item,5,Mora0dias) M0Res,
dbo.fduBSMetaRes(4,@ultdia,item,2,Saldo) MetaxDia  
from (
SELECT zon.zona 'Item', zon.nombre Oficina, sum(bs.saldocartera) Saldo, sum(bs.saldo0dias) Saldo0dias, 
...
INNER JOIN tclzona zon ON zon.zona=ofi.zona
WHERE (Fecha = @Fecha)
group by zon.zona, zon.nombre) a
order by item
...
FROM (
SELECT item, Oficina, Saldo, dbo.fduBSIndRes(4,@ultdia,item,2,Saldo) SaldoRes,  
Mora90dias, dbo.fduBSIndRes(4,@ultdia,item,6,Mora90dias) M90Incre, Mora0dias, dbo.fduBSIndRes(4,@ultdia,item,5,Mora0dias) M0Incre  
from (
SELECT zon.zona 'Item', zon.nombre Oficina, sum(bs.saldocartera) Saldo, sum(bs.saldo0dias) Saldo0dias, 
sum(bs.saldo90dias) Saldo90dias, sum(bs.saldo0dias)/sum(bs.saldocartera) * 100 Mora0dias, 
sum(bs.saldo90dias)/sum(bs.saldocartera) * 100 Mora90dias
...
INNER JOIN tclzona zon ON zon.zona=ofi.zona
WHERE (Fecha = @mesant)
group by zon.zona, zon.nombre

) a ) B inner join #tmpind on B.item=#tmpind.item

select item, saldo, round(SaldoMeta/ @dias, 2) MetaXdia,saldores, case when saldo<saldoant then '-' else '+' end + saldoincre difsaldo,saldo0dias,saldo90dias,
