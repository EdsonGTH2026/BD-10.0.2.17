...
GO
CREATE procedure [dbo].[pCsCACobranzaPuntual] @fecha smalldatetime, @fecini smalldatetime
as
...
insert into #ptmos
--select distinct codprestamo,diasmora,codoficina--codasesor,
--from tcaprestamos with(nolock)
--where estado not in('CASTIGADO','CANCELADO','ANULADO','TRAMITE','APROBADO')
--and codoficina not in('97','230','231')
--and codprestamo not in (select codprestamo from tCsCarteraAlta with(nolock))
--union
--select distinct codprestamo,diasmora,codoficina--codasesor,
--from tcaprestamos with(nolock)
--where estado='CANCELADO' and fechaultpago=@fecha
--and codoficina not in('97','230','231')
--and codprestamo not in (select codprestamo from tCsCarteraAlta with(nolock))
...
where c.fecha=@fecha---1 --and c.codprestamo in (select codprestamo from #ptmos with(nolock))
and cartera='ACTIVA'
--select count(*) from #ptmos
insert into #ptmos
...
,cu.estadocuota
--from tcacuotas cu with(nolock)
--inner join tcacuotascli cuc with(nolock) on cuc.codprestamo=cu.codprestamo and cuc.numeroplan=cu.numeroplan and cuc.seccuota=cu.seccuota
...
,cu.estadocuota
,p.codoficina
--select top 10 * from tcspadronplancuotas with(nolock)
...
GRANT EXECUTE ON [dbo].[pCsCACobranzaPuntual] TO [marista]
GO

GRANT EXECUTE ON [dbo].[pCsCACobranzaPuntual] TO [mchavezs2]
GO

GRANT EXECUTE ON [dbo].[pCsCACobranzaPuntual] TO [ope_lvegav]
GO

GRANT EXECUTE ON [dbo].[pCsCACobranzaPuntual] TO [ope_lcoronas]
GO