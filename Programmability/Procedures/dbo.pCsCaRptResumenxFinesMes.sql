...
GO
CREATE PROCEDURE [dbo].[pCsCaRptResumenxFinesMes] @FecIni smalldatetime, @FecFin smalldatetime, @Tipo int AS
...
SET @csql = @csql + ' RecuperacionAnt - Recuperacion  AS RecuperacionDif, EstimacionAnt - Estimacion AS EstimacionDif FROM ( '
...
SET @csql = @csql + 'ISNULL(B.Saldo0Dias, 0) AS Saldo0DiasAnt, ISNULL(B.Saldo90Dias, 0) AS Saldo90DiasAnt, ISNULL(B.Recuperacion, 0) AS RecuperacionAnt,ISNULL(B.Estimacion, 0) AS EstimacionAnt  '
...
SET @csql = @csql + 'AS Saldo90Dias, SUM(tCsBsCartera.Recuperacion) AS Recuperacion, SUM(tCsBsCartera.Estimacion) AS Estimacion, tClOficinas.NomOficina, tClZona.Nombre '
...
SET @csql = @csql + 'WHERE   tCsBsCartera.codoficina not in (1,99)  and   '
...
SET @csql = @csql + 'GROUP BY tCsBsCartera.Fecha, tCsBsCartera.CodOficina, tClOficinas.NomOficina, tClZona.Nombre) A LEFT OUTER JOIN '
SET @csql = @csql + '(SELECT Fecha, CodOficina, MontoDesembolso, SaldoCartera, Saldo0Dias, Saldo90Dias, Recuperacion, Estimacion, NomOficina, Nombre '
SET @csql = @csql + 'FROM (SELECT     tCsBsCartera.Fecha, tCsBsCartera.CodOficina, SUM(tCsBsCartera.MontoDesembolso) AS MontoDesembolso,  '
...
SET @csql = @csql + 'SUM(tCsBsCartera.Estimacion) AS Estimacion, tClOficinas.NomOficina, tClZona.Nombre FROM tClZona INNER JOIN tClOficinas ON tClZona.Zona = tClOficinas.Zona RIGHT OUTER JOIN '
...
SET @csql = @csql + 'GROUP BY tCsBsCartera.Fecha, tCsBsCartera.CodOficina, tClOficinas.NomOficina, tClZona.Nombre) A) B ON A.CodOficina = B.CodOficina AND B.Fecha = '
