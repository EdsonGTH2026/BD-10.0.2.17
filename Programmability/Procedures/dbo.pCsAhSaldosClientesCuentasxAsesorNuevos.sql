...
GO
CREATE PROCEDURE [dbo].[pCsAhSaldosClientesCuentasxAsesorNuevos] @Fecha smalldatetime, @FecIni smalldatetime, @FecFin smalldatetime AS
...
FROM         (SELECT     tCsAhorros.Fecha, tClOficinas.NomOficina, Asesores.NombreCompleto AS Asesor, tCsAhorros.codusuario, tCsAhorros.CodCuenta, 
                       ...
                       WHERE      (tCsAhorros.Fecha = @Fecha) AND (tCsAhorros.FechaApertura >= @FecIni AND tCsAhorros.FechaApertura <= @FecFin)) a
