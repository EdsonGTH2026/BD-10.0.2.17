...
GO
CREATE procedure [dbo].[pXaProcesoSolColDelDiaxProV2] @Oficinas VARCHAR(2000)
as
...
exec [10.0.2.14].finmas.dbo.pXaProcesoSolColDelDiaxProV2 @Oficinas
...
from (
	...
	from (
		...
		(CASE WHEN pr.Paterno IS NULL THEN pr.Materno 
		      WHEN pr.Paterno = '' THEN pr.Materno 
			  ELSE pr.Paterno END + ' ' + pr.Nombres) AS promotor
		,count(case when proceso='solicitud' then codigo else null end) sol_nro
		,sum(case when proceso='solicitud' then monto else 0 end) sol_monto
		,count(case when proceso='credito' then codigo else null end) cre_nro
		,sum(case when proceso='credito' then monto else 0 end) cre_monto
		...
		group by z.nombre,o.nomoficina, pr.Paterno, pr.Materno, pr.Nombres --pr.nombrecompleto
	) a
	--left outer join (
...
where o.tipo<>'Cerrada'
and o.codoficina not in(99,98,97)
AND o.CodOficina IN (SELECT Codigo FROM dbo.fduTablaValores(@Oficinas))
...
left outer join (
		...
		from tcsempleados e  with(nolock) inner join tcloficinas o with(nolock) on o.codoficina=e.codoficinanom
		where estado=1 and codpuesto=66
		AND e.CodOficinaNom IN (SELECT Codigo FROM dbo.fduTablaValores(@Oficinas))
		group by o.nomoficina
...
from (
	select o.nomoficina sucursal,z.nombre region,
	(CASE WHEN cl.Paterno IS NULL THEN cl.Materno 
		  WHEN cl.Paterno = '' THEN cl.Materno 
		  ELSE cl.Paterno END + ' ' + cl.Nombres) AS promotor
	from tcsempleados e  with(nolock) inner join tcloficinas o with(nolock) on o.codoficina=e.codoficinanom
	inner join tcspadronclientes cl with(nolock) on cl.codusuario=e.codusuario
	inner join tclzona z with(nolock) on z.zona=o.zona	
	where estado=1 and codpuesto=66
	AND e.CodOficinaNom IN (SELECT Codigo FROM dbo.fduTablaValores(@Oficinas))
	group by o.nomoficina,z.nombre, cl.Paterno, cl.Materno, cl.Nombres --cl.nombrecompleto
) a
