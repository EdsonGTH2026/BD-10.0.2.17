...
GO
--select * from tcloficinas where nomoficina like '%campeche%'
--pCsCuadroCASucursalMes '20171130'
CREATE procedure [dbo].[pCsCuadroCASucursalMes] @fecfin datetime
as
...
inner join (
	...
	,sum(cd.saldocapital) saldocapital,sum(interesvigente+interesvencido) interes
...
inner join (
	...
	,sum(interesvigente+interesvencido) interes
	,sum(cd.moratoriovigente) + sum(cd.moratoriovencido) moratorio
	FROM tCsCartera c with(nolock) 
	inner join tcscarteradet cd with(nolock) on c.fecha=cd.fecha and c.codprestamo=cd.codprestamo  
	where c.cartera='ACTIVA' and c.fecha=@fecfin_a--'20161231' --
...
inner join (
	...
	,sum(cd.saldocapital) saldocapital,sum(interesvigente+interesvencido) interes,sum(interesctaorden) ctaorden
	,sum(cd.moratoriovigente) + sum(cd.moratoriovencido) moratorio
	FROM tCsCartera c with(nolock) 
	inner join tcscarteradet cd with(nolock) on c.fecha=cd.fecha and c.codprestamo=cd.codprestamo  
...
update #cuadro
set ingrealint=interes,ingmoratorio=moratorios,ingseguros=seguros
from #cuadro a
inner join (
	select --case when (cast(codoficinacuenta as int)<100 or cast(codoficinacuenta as int)>300) then codoficinacuenta else cast((cast(codoficinacuenta as int)+200) as varchar(4)) end codoficina
		case when codoficinacuenta='150' then codoficinacuenta
	...
	,sum(montointerestran) interes,sum(montoinpetran+montocargos) moratorios,sum(MontoOtrosTran) seguros
	from tcstransacciondiaria t with(nolock)
...
--delete from #cuadro where scapitalini=0 and scapitalfin=0 and ingint=0 and scapitalvenfin=0 and scapitalvenini=0 and sinteresvenini=0
...
GRANT EXECUTE ON [dbo].[pCsCuadroCASucursalMes] TO [marista]
GO