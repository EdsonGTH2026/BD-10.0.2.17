...
--set @T2 = getdate()
--print 'T1 '+ cast( datediff(millisecond, @T1, @T2) as varchar(30))
--set @T1 = getdate()
...
and (codasesor=@codasesor or @codasesor='' or @codasesor is null)
--select * from #ptmos
--set @T2 = getdate()
--print 'T2 '+ cast( datediff(millisecond, @T1, @T2) as varchar(30))
--set @T1 = getdate()

select --sucursal,--,count(distinct codusuario) nroclie
count(distinct codprestamo) nroptmo
,isnull(sum(saldocapital),0) saldocapital
,isnull(sum(saldocartera),0) saldocartera
,isnull(sum(deuda),0) saldodeuda
,count(distinct D0nroptmo) D0nroptmo,isnull(sum(D0saldo),0) D0saldo, isnull((sum(D0saldo)/sum(saldocapital))*100,0) D0Por
,count(distinct D1a7nroptmo) D1a7nroptmo,isnull(sum(D1a7saldo),0) D1a7saldo, isnull((sum(D1a7saldo)/sum(saldocapital))*100,0) D1a7Por
,count(distinct D8a15nroptmo) D8a15nroptmo,isnull(sum(D8a15saldo),0) D8a15saldo, isnull((sum(D8a15saldo)/sum(saldocapital))*100,0) D8a15Por
,count(distinct D16a30nroptmo) D16a30nroptmo,isnull(sum(D16a30saldo),0) D16a30saldo, isnull((sum(D16a30saldo)/sum(saldocapital))*100,0) D16a30Por
,count(distinct D31a60nroptmo) D31a60nroptmo,isnull(sum(D31a60saldo),0) D31a60saldo, isnull((sum(D31a60saldo)/sum(saldocapital))*100,0) D31a60Por
,count(distinct D61a89nroptmo) D61a89nroptmo,isnull(sum(D61a89saldo),0) D61a89saldo, isnull((sum(D61a89saldo)/sum(saldocapital))*100,0) D61a89Por
,count(distinct D90a239nroptmo) D90a239nroptmo,isnull(sum(D90a239saldo),0) D90a239saldo, isnull((sum(D90a239saldo)/sum(saldocapital))*100,0) D90a239Por
,count(distinct Dm240nroptmo) Dm240nroptmo,isnull(sum(Dm240saldo),0) Dm240saldo, isnull((sum(Dm240saldo)/sum(saldocapital))*100,0) DM240Por
,count(distinct Progrenroptmo) Progrenroptmo,isnull(sum(Progresaldo),0) Progresaldo, isnull((sum(Progresaldo)/sum(saldocapital))*100,0) ProgrePor
,count(distinct Propionroptmo) Propionroptmo,isnull(sum(Propiosaldo),0) Propiosaldo, isnull((sum(Propiosaldo)/sum(saldocapital))*100,0) PropioPor

,count(distinct D0a90nroptmo) D0a90nroptmo,isnull(sum(D0a90saldo),0) D0a90saldo, isnull((sum(D0a90saldo)/sum(saldocapital))*100,0) D0a90Por
,count(distinct DM90nroptmo) DM90nroptmo,isnull(sum(DM90saldo),0) DM90saldo, isnull((sum(DM90saldo)/sum(saldocapital))*100,0) DM90Por
