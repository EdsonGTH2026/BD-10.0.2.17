...
GO
CREATE procedure [dbo].[pCsRptCaAfectanPorPromotor] (@codpromotor varchar(250))
as
...
insert into #PtmoCER (item,descripcion) values(1,'De 4 a 59 Días')
insert into #PtmoCER (item,descripcion) values(2,'De 60 y más Días')
...
--select @valores
and c.codprestamo in (select codprestamo from tCsCaCosechasBase with(nolock) where promotorreporte=@codpromotor)
*/
--NEW
...
and b.nrodiasatraso>=4 and b.nrodiasatraso<=59 
and b.Desembolso>='20140101' and b.Desembolso<='20141231'
and b.codprestamo in (select codprestamo from tCsCaCosechasBase with(nolock) where promotorreporte=@codpromotor)
