...
GO
create procedure [dbo].[pCosechas_VintageCrear_Modificado_Alta] ( @Ubicacion Varchar(100), @ClaseCartera Varchar(100))
as
...
--set @ClaseCartera		= 'TODAS'	-- "ACTIVA", "CASTIGADA", "ADMINISTRATIVA"
...
IF @Ubicacion = 'ZZZ'
BEGIN
Exec pGnlCalculaParametros3 1, @Ubicacion, 		@CUbicacion 	Out, 	@Ubicacion 		Out,  @OtroDato Out
Exec pGnlCalculaParametros3 2, @ClaseCartera, 	@CClaseCartera 	Out, 	@ClaseCartera 	Out,  @OtroDato Out
SET @CUbicacion = (SELECT SUBSTRING(@CUbicacion,1,218)+ ', 0')
END
ELSE 
BEGIN
Exec pGnlCalculaParametros3 1, @Ubicacion, 		@CUbicacion 	Out, 	@Ubicacion 		Out,  @OtroDato Out
...
--Select @FG	= dbo.fduFechaATexto('20130701', 'AAAAMM') From vCsFechaConsolidacion ----DIA SIGUIENTE A LA FECHA LIMITE O CIERRE
	              ...
	              ,tCsCartera.saldo --> 619 registros
				        FROM (
				              ...
				              FROM (
				                    ...
				                    FROM (
				                          ...
				                          WHERE CodPrestamo IN (Select Codprestamo from #Temporal)
				        ...
				        (
				         ...
				         FROM (
				               ...
				               AND (dbo.fduFechaATexto(Fecha, 'AAAAMM') >= @PI)
				               ) Datos
...
FROM (SELECT Datos.Ubicacion, Datos.Cartera, Datos.Periodo, MAX(tCsVintage_Nuevo.Proceso) AS Proceso
      FROM (SELECT DISTINCT Ubicacion, Cartera, Periodo
            ...
            WHERE (Ubicacion = @PUbicacion) AND (Cartera = @PClaseCartera) AND (Proceso IS NULL)) Datos 
