...
/****** Object:  Stored Procedure dbo.pCsRCGeneraDatosVr14    Script Date: 08/03/2023 09:06:02 pm ******/
CREATE Procedure [dbo].[pCsRCGeneraDatosVr14]
...
Declare curEstructuraArchivo Cursor For 
	...
	WHERE     (TipoArchivoTexto = @TipoArchivoTexto) And Activo = 1--@TipoArchivoTexto  --OSC: cambio de 001 a 006
Open curEstructuraArchivo
Fetch Next From curEstructuraArchivo Into @EstructuraArchivo, @Tabla, @TablaRecorrido
While @@Fetch_Status = 0
Begin 
    if @ShowPrint = 1 PRINT ''
    if @ShowPrint = 1 PRINT '<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'
	if @ShowPrint = 1 PRINT '<<<<<<< EstructuraArchivo:' + @EstructuraArchivo + ', Tabla:' + @Tabla + ', TablaRecorrido:' + @TablaRecorrido
	if @ShowPrint = 1 PRINT ''
	...
	Begin
	    if (@TablaRecorrido='vINTFNombreVr14')
			begin
			  ...
			  if @ShowPrint = 1 Print 'Ejecuta: ' + @Cadena
					    Exec (@Cadena)
			end
	    if (@TablaRecorrido='vINTFCuentaVr14')
			begin
			  --exec finamigoconsolidado.dbo.pvINTFCuenta '20121231'
			  Set @Cadena = 'exec finamigoconsolidado.dbo.pvINTFCuentavr14 '''+dbo.fduFechaAtexto(@Fecha, 'AAAAMMDD')+''' '
			  if @ShowPrint = 1 Print 'Ejecuta: ' + @Cadena
					    Exec (@Cadena)
			end	  
	...
	if @ShowPrint = 1 print 'Tabla Recorrido: ' + @TablaRecorrido
	...
	if @ShowPrint = 1 print '***********   Inicia crufracmento  *****************'	
	...
	Begin 		
		...
		Begin
			...
			Begin
				if @ShowPrint = 1 Print 'OBSERVACION('+ @campo +'): Tener cuenta valor ya que el tamaño es inconsistente'
				if @ShowPrint = 1 Print '-- Valor 	: ' + @Valor
				if @ShowPrint = 1 Print '-- Len		: ' + Cast(Len(@Valor) as Varchar(10))
				if @ShowPrint = 1 Print '-- DataLength	: ' + Cast(DataLength(@Valor) as Varchar(10))
			End
		End				
		...
		if @ShowPrint = 1 Print 'Valor Inicial	:' + @Valor
		if @ShowPrint = 1 Print 'Tilde		:' + Cast(@Tilde as Varchar(1))
		If @Tilde = 0 
		Begin	
			Set @CS = 'A'
			Set @CC = 'Á'		
		...
		End 
		if @ShowPrint = 1 Print 'Tipo de Dato = '  + @TipoDato 
		If @TipoDato = 'C'
		Begin
			Set @Valor = '''' + @Valor + ''', ' 
		End
		...
		if @ShowPrint = 1 Print 'Campo = ' + @Campo + ' : ' + IsNull(@Valor, 'Valor Nulo')

		Set @Valores	= @Valores 	+ @Valor

		--Print @Valores
		if @ShowPrint = 1 Print 'Contador: ' + convert(varchar, @Contador)
		if @ShowPrint = 1 Print 'Orden: ' + convert(varchar, @Orden)
		If @Contador = @Orden
		Begin
			...
			if @ShowPrint = 1 Print @Tabla
			if @ShowPrint = 1 Print @Campos
			if @ShowPrint = 1 Print @Periodo
			if @ShowPrint = 1 Print @Fila
			if @ShowPrint = 1 Print @CadenaD
			if @ShowPrint = 1 Print @Valores
			if @ShowPrint = 1 Print '@Cadena : ' + Isnull(@Cadena, '') 	
			Exec (@Cadena)
	...
	set @T2 = getdate()
	if @ShowPrint = 1 print 'Tiempo '+ cast( datediff(millisecond, @T1, @T2) as char(6)) + ' termina curfracmento'
	set @T1 = getdate()	
