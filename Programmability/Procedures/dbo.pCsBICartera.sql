...
GO
CREATE procedure [dbo].[pCsBICartera] 
as
...
,case when c.cartera='CASTIGADA' then cd.saldocapital else 0 end Castigadosaldo
  ...
  union 
    ...
    SELECT pd.secuenciacliente,c.Fecha,cd.codusuario,c.CodPrestamo, o.nomoficina, case when o.esvirtual = 1 then 'Virtual' else 'Fisica' end tipo
    ,z.nombre region
  ,cd.saldocapital  
  ,c.CodAsesor
  ,case when (e.codusuario is null or e.codpuesto<>66) then 'HUERFANO' else pc.nombrecompleto end promotor
...
,cd.saldocapital Castigadosaldo
  ...
  select tf.fecha FECHA,tf.nomoficina SUCURSAL, tf.tipo ,tf.region REGION,tf.saldocapital SALDOCAPITALFIN, tf.castigadosaldo SALDOCASTIGADO
