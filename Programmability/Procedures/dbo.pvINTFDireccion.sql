...
/****** Object:  Stored Procedure dbo.pvINTFDireccion    Script Date: 08/03/2023 09:06:02 pm ******/
--exec pvINTFDireccion
--CREATE view [dbo].[vINTFDireccionVr14]
--as
--00:01:24
CREATE procedure [dbo].[pvINTFDireccion]
as
truncate table tCsTblDirecciones

insert into tCsTblDirecciones
SELECT DISTINCT 
Datos.CodUsuario, 
...
FROM (SELECT     CodUsuario, 
      ...
      FROM (
				...
				FROM (
					SELECT     *
					FROM vINTFDireccionFamiliarVr14  with(nolock)
					UNION
					SELECT     *
					FROM vINTFDireccionNegocioVr14  with(nolock)
				) Datos
				inner join (
