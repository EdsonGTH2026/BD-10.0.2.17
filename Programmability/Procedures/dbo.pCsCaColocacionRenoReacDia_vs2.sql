...
GO
--pCsCaColocacionRenoReacDia '20190328','15,21,3,301,101,302,102,303,103,304,104,307,107,308,108,309,109,310,110
--,311,111,315,115,317,117,318,118,319,119,320,120,321,121,322,122,323,123,324,124,325,125,326,126,327,127,329,129,33,330,130,332,132,333,133,334,134,335,135
--,336,136,337,137,339,139,341,141,342,142,344,144,37,131,4,41,430,431,432,232,433,233,5,6,8,25,114,28'
CREATE procedure [dbo].[pCsCaColocacionRenoReacDia_vs2] @fecha smalldatetime,@codoficina varchar(2000)
as
--declare @fecha smalldatetime
--declare @codoficina varchar(500)
--set @fecha='20200921'
--set @codoficina= '15,21,3,301,101,302,102,303,103,304,104,307,107,308,108,309,109,310,110,311,111,315,115,317,117,318,118,319,119
--,320,120,321,121,322,122,323,123,324,124,325,125,326,126,327,127,329,129,33,330,130,332,132,333,133,334,134,335,135,336,136,337,137
--,339,139,341,141,342,142,344,144,37,131,4,41,430,431,432,232,433,233,5,6,8,25,114,28'

set nocount on
create table #Ptmos (codprestamo varchar(25) not null,codusuario varchar(15),desembolso smalldatetime,monto money,codproducto char(3),codfondo int,tiporeprog varchar(15))
insert into #Ptmos 
--exec [10.0.2.14].finmas.dbo.pCsCaColocacionRenoReacDia2 @fecha,@codoficina
select codprestamo,codusuario,fechadesembolso,montodesembolso,codproducto,codfondo,isnull(tiporeprog,'SINRE') tiporeprog
...
,sum(case when dbo.fdufechaaperiodo(p.desembolso)=dbo.fdufechaaperiodo(l.cancelacion) or p.tiporeprog='RENOV' then 
	--p.monto 
		case when p.tiporeprog='SINRE' then p.monto else 0 end
...
,count(case when dbo.fdufechaaperiodo(p.desembolso)=dbo.fdufechaaperiodo(l.cancelacion) or p.tiporeprog='RENOV' then 
	--p.codusuario 
		case when p.tiporeprog='SINRE' then p.codusuario else null end
...
,sum(case when p.codfondo=20 then p.monto*0.3 when p.codfondo=21 then p.monto*0.25 else p.monto end) propio
,sum(case when p.codfondo=20 then p.monto*0.7 else 0 end) progresemos
,sum(case when p.codfondo=21 then p.monto*0.75 else 0 end) cubo
from #Ptmos p with(nolock)
