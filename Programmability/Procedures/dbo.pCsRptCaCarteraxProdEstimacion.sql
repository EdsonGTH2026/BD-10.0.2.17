...
SET ANSI_NULLS OFF
GO
--drop procedure [pCsRptCaCarteraxProdEstimacion]
CREATE procedure [dbo].[pCsRptCaCarteraxProdEstimacion] @fecha smalldatetime
as
--declare @fecha smalldatetime
--set @fecha='20140916'

select codproducto,producto
,count(distinct codusuario) nroclie, count(distinct codprestamo) nroptmo
...
from (
  SELECT pc.codproducto
  ,pc.Nombreprod producto
  ...
  ,cd.sreservacapital + cd.sreservainteres t_saldo
  ,cd.saldocapital + cd.interesvigente+cd.interesvencido+cd.moratoriovigente+cd.moratoriovencido montodesembolso
  ...
  ,case when c.Estado<>'VENCIDO' then
    ...
    then cd.sreservacapital + cd.sreservainteres--cd.saldocapital + cd.interesvigente+cd.interesvencido+cd.moratoriovigente+cd.moratoriovencido
    else 0 end
   else 0 end D0saldo
  ,case when c.Estado<>'VENCIDO' then
  ...
  ,case when c.Estado='VENCIDO' then cd.sreservacapital + cd.sreservainteres--cd.saldocapital + cd.interesvigente+cd.interesvencido+cd.moratoriovigente+cd.moratoriovencido
  ...
  where c.cartera='ACTIVA' and c.fecha=@fecha and c.codoficina<99
) a
