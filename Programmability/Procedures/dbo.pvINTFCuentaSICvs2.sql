...
----SP_HELPTEXT [pvINTFCuentaSICvs2]
CREATE PROCEDURE [dbo].[pvINTFCuentaSICvs2] @fecha smalldatetime               
AS              
...
INSERT INTO #DEPURADOS (CODPRESTAMO) VALUES ('098-170-06-02-00008');
...
insert into tCsBuroxTblReICueVr14  --------------------------------------------------------------------------------------------OJO            
...
/*FROM <----------*/              
--select c.*              
--into #TEMPtCsBuroxTblReICueVr14  --------------------------------------------------------------------------------------------OJO            
...
--left outer join (  --------------------------------------------------------------se saca a temporal para NO usar una subconsulta ZCCU            
--        SELECT CodPrestamo, COUNT(*) AS PagosReportados, SUM(MOP01) AS MOP01, SUM(MOP02) AS MOP02, SUM(MOP03) AS MOP03, SUM(MOP04)               
--        AS MOP04, SUM(MOP05) AS MOP05              
--        FROM (              
--            SELECT CodPrestamo, CodUsuario, SecCuota, CASE MOP WHEN '01' THEN 1 ELSE 0 END AS MOP01,               
...
insert into tCsBuroxTblReICueVr14 --------------------------------------------------------------------------------------------OJO            
--INSERT INTO #TEMPtCsBuroxTblReICueVr14  --------------------------------------------------------------------------------------------OJO            
----************************************ AVALES ****************************************              
SELECT v.CodPrestamo, v.CodUsuario, vINTFCabecera.ClaveUsuario, vINTFCabecera.NombreUsuario              
...
update tCsBuroxTblReICueVr14-----#TEMPtCsBuroxTblReICueVr14             
...
from tCsBuroxTblReICueVr14 as cue with(nolock)  ------------------------------------------------------------------------------OJO            
...
--select * from #TEMPtCsBuroxTblReICueVr14 with(nolock)            
...
from tCsBuroxTblReICueVr14
where  MontoPagar>SaldoActual     
------------------------------------------------------------------------------------------------------------------------------------            
            
--------------------------------DESCOMENTAR Y USAR PARA PRUEBAS--------------------------------------------------------------------------OJO            
  --#TEMPtCsBuroxTblReICueVr14             
--update #TEMPtCsBuroxTblReICueVr14            
-- set  MOP = '02'              
--where SaldoVencido > 0               
--and MOP in ('00','UR', '01' )               
--and Cancelacion = ''             
...
--DROP TABLE #TEMPtCsBuroxTblReICueVr14 --- OJO            
DROP TABLE #tblmop_tmp            
...
--SELECT COUNT(*) FROM #TEMPtCsBuroxTblReICueVr14 WITH (NOLOCK) ------------------------OJO            
--SELECT COUNT(*) FROM tCsBuroxTblReICueVr14 WITH (NOLOCK)     -------------------------OJO            
            
            
--56,090  ----54,804            
SELECT  COUNT(distinct CODPRESTAMO) FROM #TEMPtCsBuroxTblReICueVr14 WITH (NOLOCK) --------------------------------------------------------------------------------------------OJO            
            
            
--60710--////55,965 2023.05///-------------------58,813            
SELECT COUNT(distinct CODPRESTAMO) --select *             
FROM tCsBuroxTblReICueVr14 WITH (NOLOCK) where substring(codprestamo,1,3)<>'500' --------------------------------------------------------------------------------------------OJO            
...
---------------------4,009            
SELECT COUNT(distinct CODPRESTAMO) --select *             
FROM tCsBuroxTblReICueVr14 c WITH (NOLOCK)  --------------------------------------------------------------------------------------------OJO            
