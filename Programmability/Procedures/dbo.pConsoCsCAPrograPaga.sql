...
CREATE procedure [dbo].[pConsoCsCAPrograPaga]  
as  
set nocount on  
...
union 
---se agrega 2023.02.04  CCU -- Para evitar NULLS EN CODASESOR  
select codprestamo,isnull(ultimoasesor,primerasesor),codoficina,0 nrodiasatraso,'LIQUIDADO',secuenciacliente,codproducto  
from tcspadroncarteradet with(nolock)  
where cancelacion>=@fecini and cancelacion<=@fecha 
and codoficina <>'999' 
--AND ULTIMOASESOR IS NULL
union  
