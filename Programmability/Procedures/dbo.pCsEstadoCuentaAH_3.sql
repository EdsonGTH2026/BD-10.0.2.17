...
---exec pCsEstadoCuentaAH_3 'riglesiasa2','098-209-06-2-0-00001-0-0','2020-03-01','2020-03-31'




CREATE PROCEDURE   [dbo].[pCsEstadoCuentaAH_3]   @Usuario     VARCHAR(50), 
                                             @Cuenta      VARCHAR(25), 
...
----set @UltimoCorte='20180118'  
     ...
     SELECT @PrimerCorte AS Inicio, 
            ...
            @Firma AS Firma, 
            CodPrestamo = @Cuenta, 
            tCsAhorros.CodUsuario AS CodUsuario, 
   ...
   GROUP BY CodPrestamo
    ...
    --drop table #Saldos                                                                                                   AND tCsAhorros.Renovado = tCsPadronAhorros.Renovado
     INNER JOIN
     (
         SELECT tCsAhorros_1.CodCuenta, 
                tCsAhorros_1.FraccionCta, 
                tCsAhorros_1.Renovado, 
               ---tCsAhorros_1.CodUsuario AS CodUsuario, 
			    tCsAhorros_1.CodUsuario AS CodUsuario, 
         ...
         (
             ...
             (
                 SELECT tCsAhorros_2.CodCuenta, 
					    ...
					    tCsAhorros_2.CodUsuario AS CodUsuario,
                       -- tCsClientesAhorrosFecha.CodUsCuenta AS CodUsuario, 
                        ISNULL(tCsPadronClientes_1.NombreCompleto, '') AS ClienteGrupo
              
			  FROM tCsTransaccionDiaria WITH(NOLOCK)
                      INNER JOIN tCsAhorros AS tCsAhorros_2 WITH(NOLOCK) ON tCsTransaccionDiaria.Fecha = tCsAhorros_2.Fecha
                                                                            AND tCsTransaccionDiaria.CodigoCuenta = tCsAhorros_2.CodCuenta
                                                                            AND tCsTransaccionDiaria.FraccionCta = tCsAhorros_2.FraccionCta
                                                                            AND tCsTransaccionDiaria.Renovado = tCsAhorros_2.Renovado
     ...
     INNER JOIN tCsAhorros AS tCsClientesAhorrosFecha_2 WITH(NOLOCK) ON General.CodCuenta = tCsClientesAhorrosFecha_2.CodCuenta
                                                                                     AND General.FraccionCta = tCsClientesAhorrosFecha_2.FraccionCta
                                                                                     AND General.Renovado = tCsClientesAhorrosFecha_2.Renovado
                                                                                     AND General.CodUsuario = tCsClientesAhorrosFecha_2.CodUsuario   
         --INNER JOIN  
     LEFT JOIN tCsPadronClientes WITH(NOLOCK) ON tCsClientesAhorrosFecha_2.CodUsuario = tCsPadronClientes.CodUsuario
     INNER JOIN tAhClFormaManejo WITH(NOLOCK) ON tCsClientesAhorrosFecha_2.FormaManejo = tAhClFormaManejo.FormaManejo
	 ON tCsAhorros.Fecha = tCsClientesAhorrosFecha_2.Fecha
      ...
      AND tCsAhorros.Renovado = tCsClientesAhorrosFecha_2.Renovado 
	  ON Cargos.CodPrestamo = tCsPadronAhorros.CodCuenta + '-' + tCsPadronAhorros.FraccionCta + '-' + CAST(tCsPadronAhorros.Renovado AS VARCHAR(5)) 
	  ON Abonos.CodCuenta = tCsPadronAhorros.CodCuenta
                                                                                                                                                                                                                                                               
                                                                        AND Abonos.FraccionCta = tCsPadronAhorros.FraccionCta
                                                                                                                                                                                                                                                               
                                                                        AND Abonos.Renovado = tCsPadronAhorros.Renovado --ON tCsPadronAhorros.CodCuenta + '-' + tCsPadronAhorros.FraccionCta + '-' + CAST(tCsPadronAhorros.Renovado AS varchar(5)) = Vigente.Co
