...
--exec pvINTFNombreSIC_CP '20230801','20230805'  
--sp_helptext pvINTFNombreSIC_CP

CREATE PROCEDURE [dbo].[pvINTFNombreSIC_CP] @primerdia smalldatetime,@fecha smalldatetime       
AS    
set nocount on    
...
--select @primerdia='20230801'  --comentar primerdia from tclperiodo with(nolock) where ultimodia=@fecha    
    
declare @feciniMes smalldatetime  
SET @feciniMes = DBO.FDUFECHAAPERIODO(@FECHA)+'01' ---- FECHA DE INICIO DE MES       
  
  ---------------tCsBuroxTblReInomVr14CP  
truncate table FinamigoConsolidado.dbo.tCsBuroxTblReInomVr14CP --descomentar   -------------------------------------------------------------------------------------OJO  
--truncate table   dev_cchavezd.TempCCTblReINombrev14  -------------------------------------------------------------------------------------OJO  
...
where d.Fecha=@fecha    
and d.codprestamo not in (select codprestamo from tCsBuroDepuLey with(nolock))  
and d.codprestamo not in (select codcuenta from tcsCCDepurados2023 with(nolock))  
...
and c.fechadesembolso<@feciniMes ---- no se envian créditos nuevos, sin un alta previa. (NO DESEMBOLSADOS EN EL MES)  
----select * from ca#    
----drop table ca#    
  
insert into FinamigoConsolidado.dbo.tCsBuroxTblReInomVr14CP    -------------------------------------------------------------------------------------OJO  
--insert into dev_cchavezd.TempCCTblReINombrev14     -------------------------------------------------------------------------------------OJO  
...
--select TOP 1000* from FinamigoConsolidado.dbo.tCsBuroxTblReInomVr14CP with(nolock)  
--select count(codprestamo) from FinamigoConsolidado.dbo.tCsBuroxTblReInomVr14CP with(nolock)  
---mensual 75,400  
  
  
--select TOP 1000*   
--into TempCCTblReINombrev14  
--from tCsBuroxTblReInomVr14 with(nolock)  
  
  
--select count(codprestamo) from dev_cchavezd.TempCCTblReINombrev14 with(nolock)  
  
--select TOP 1000* from dev_cchavezd.TempCCTblReINombrev14 with(nolock)  
GO