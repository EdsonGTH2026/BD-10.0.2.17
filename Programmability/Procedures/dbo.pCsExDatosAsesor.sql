...
GO
CREATE procedure [dbo].[pCsExDatosAsesor] @fecha smalldatetime,@codasesor varchar(15)
as 
...
select sucursal,count(distinct codusuario) nroclie, count(distinct codprestamo) nroptmo
...
from (
  ...
  ,case when c.NroDiasAtraso>0 then cd.saldocapital + cd.interesvigente+cd.interesvencido+cd.moratoriovigente+cd.moratoriovencido
    else 0 end Dm0saldo
