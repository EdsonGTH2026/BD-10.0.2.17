...
GO
create procedure [dbo].[pCsRptCaCarteraxProdxMotos] @fecha smalldatetime
as
--declare @fecha smalldatetime
--set @fecha='20140807'

select codproducto,producto,productodestino
...
from (
  ...
  ,isnull(d.DescDestino,pc.nombreprod) productodestino
  ...
  left outer join tCsPadronCarteraDestinoDet pdd on pdd.codprestamo=cd.codprestamo and pdd.codusuario=cd.codusuario
  left outer join tcacldestino d on d.coddestino=pdd.coddestino
  inner join tcaproducto pc on pc.codproducto=isnull(op.codproducto,c.codproducto)
  where c.cartera='ACTIVA' and c.fecha=@fecha
) a
group by codproducto,producto,productodestino
GO