...
GO
CREATE PROCEDURE [dbo].[pCsCtaCatalogoMinimo] @FechaIni smalldatetime, @FechaFin smalldatetime AS
SET NOCOUNT ON
...
BEGIN
	...
	BEGIN
		...
		ELSE ISNULL(SalFinHaber,0) - ISNULL(SalFinDebe,0) END AS VALOR, ISNULL(SalFinHaber,0) SalFinHaber, ISNULL(SalFinDebe,0) SalFinDebe  FROM #tbAux WHERE (SUBSTRING(CodCta, 1, LEN(@CodCuenta)) = @CodCuenta)) A
...
/*
		DECLARE reg_calculax CURSOR FOR 
		SELECT Codigo, Operacion, CuentaCampo FROM #Plantilla
		WHERE (TipoValor = 2) AND (Basedatos = 0)
		
		OPEN reg_calculax
		
		FETCH NEXT FROM reg_calculax 
		INTO @Codigo, @Operacion, @CuentaCampo
...
--WHERE (TipoValor IN(2)) --AND (Basedatos = 1)
