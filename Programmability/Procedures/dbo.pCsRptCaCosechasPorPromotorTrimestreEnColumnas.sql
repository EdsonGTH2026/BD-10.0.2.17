...
GO
CREATE PROCEDURE [dbo].[pCsRptCaCosechasPorPromotorTrimestreEnColumnas] (@CodPromotor as varchar(250))
as
...
isnull((  	select ((sum(Saldo2)/ sum(Monto)) * 100 ) as CER@4_Saldo
        ...
        where PromotorReporte = t3.PromotorReporte and Trimestre = '1T - 2015'
        group by  PromotorReporte
    ),-0.01) as '1T 2015 Saldo',
isnull((  	select ((sum(Saldo2)/ sum(Monto)) * 100 ) as CER@4_Saldo
        from tCsCaCosechasBase 
        where PromotorReporte = t3.PromotorReporte and Trimestre = '2T - 2015'
        group by  PromotorReporte
    ),-0.01) as '2T 2015 Saldo',
isnull((  	select ((sum(Saldo2)/ sum(Monto)) * 100 ) as CER@4_Saldo
        from tCsCaCosechasBase 
        where PromotorReporte = t3.PromotorReporte and Trimestre = '3T - 2015'
        group by  PromotorReporte
    ),-0.01) as '3T 2015 Saldo',
isnull((  	select ((sum(Saldo2)/ sum(Monto)) * 100 ) as CER@4_Saldo
        from tCsCaCosechasBase 
        where PromotorReporte = t3.PromotorReporte and Trimestre = '4T - 2015'
        group by  PromotorReporte
    ),-0.01) as '4T 2015 Saldo',

isnull((  	select ((sum(Saldo2)/ sum(Monto)) * 100 ) as CER@4_Saldo
        from tCsCaCosechasBase 
        where PromotorReporte = t3.PromotorReporte and Trimestre = '1T - 2016'
        group by PromotorReporte
    ),-0.01) as '1T 2016 Saldo',
isnull((  	select ((sum(Saldo2)/ sum(Monto)) * 100 ) as CER@4_Saldo
        from tCsCaCosechasBase 
        where PromotorReporte = t3.PromotorReporte and Trimestre = '2T - 2016'
        group by  PromotorReporte
    ),-0.01) as '2T 2016 Saldo',
isnull((  	select ((sum(Saldo2)/ sum(Monto)) * 100 ) as CER@4_Saldo
        from tCsCaCosechasBase 
        where PromotorReporte = t3.PromotorReporte and Trimestre = '3T - 2016'
        group by  PromotorReporte
    ),-0.01) as '3T 2016 Saldo',
isnull((  	select ((sum(Saldo2)/ sum(Monto)) * 100 ) as CER@4_Saldo
        from tCsCaCosechasBase 
        where PromotorReporte = t3.PromotorReporte and Trimestre = '4T - 2016'
        group by  PromotorReporte
