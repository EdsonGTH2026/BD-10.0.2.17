...
GO
--DROP PROC pCsAAnalisisxSuc
--EXEC pCsAAnalisisxSuc '20140312',''
CREATE PROCEDURE [dbo].[pCsAAnalisisxSuc]
               ( @Fecha      SMALLDATETIME ,
...
IF  EXISTS (SELECT * FROM tCsRptAnalisisxSuc) --dbo.sysobjects WHERE id = OBJECT_ID(N'[tCsRptBonoCartera]'))-- AND type = 'D')    
BEGIN    
     DROP TABLE tCsRptAnalisisxSuc    
END    

SELECT @Fecha Fecha, ca.CodOficina+' '+o.NomOficina Oficina,
...
--UPDATE tSgCmInfoAuto SET Activo = 1 where idcola between 3 and 16
...
--select * from tcsempleados where materno like '%arreola%'
--INSERT INTO tSgCmInfoAuto select 17,baseini, tipop,	pini1, 'pCsAAnalisisxSucDatos', '@fecha','@codoficina',NULL,'ANXS',1  from tSgCmInfoAuto where idcola = 16
--INSERT INTO tSgCmInfoAuto select 18,baseini, tipop,	pini1, 'pCsAAnalisisxRegDatos', '@fecha','@codoficina',NULL,'ANXR',1  from tSgCmInfoAuto where idcola = 16
--INSERT INTO tSgCmInfoAuto select 19,baseini, tipop,	pini1, 'pCsAAnalisisxSucDatos', '@fecha','@codoficina',NULL,'ANXS',1  from tSgCmInfoAuto where idcola = 16
--INSERT INTO tSgCmInfoAuto select 20,baseini, tipop,	pini1, 'pCsAAnalisisxRegDatos', '@fecha','@codoficina',NULL,'ANXR',1  from tSgCmInfoAuto where idcola = 16
GO