...
GO
CREATE procedure [dbo].[pCsAgeneraTextoMasivo] @Nomarchivo varchar(100), @ExpSP varchar(700)
as

--declare @Nomarchivo varchar(100), @ExpSP varchar(200)
--set @Nomarchivo='Servando_PRUEBatxt'
--set @ExpSP ='pCsADatosxAsesorxSucursal ''''20130407'''',''''10'''''--'pCsCboOficinas'
--set @ExpSP ='pCsCaSegurosAtlasTexto ''''201402'''''

declare @f varchar(2000)

set @f='SELECT * INTO ##TExD1 '
set @f=@f+'FROM OPENROWSET(''MSDASQL'', ''DRIVER={SQL Server};SERVER=10.0.1.17;UID=sa;PWD=$sql$2013'','
...
set @cmd='bcp "Select * from [10.0.1.17].FinamigoConsolidado.dbo.##TExD1" '
set @cmd=@cmd+'queryout "C:\Reportes_eDN\Auto\'+@Nomarchivo+'.txt" -UAdmin -PFNMG$ADM_08 -c '
set @cmd=@cmd+''
--Exec Master..xp_cmdshell 'bcp "Select * from [10.0.1.17].FinamigoConsolidado.dbo.##TExD2" queryout "C:\Reportes_eDN\Auto\'+@Nomarchivo+'.xls" -UAdmin -PFNMG$ADM_08 -c' 
