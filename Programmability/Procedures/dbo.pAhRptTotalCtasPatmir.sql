...
GO
--SP_HELPTEXT pAhRptTotalCtasPatmir                    
--DROP PROC pAhRptTotalCtasPatmir                    
--EXEC pAhRptTotalCtasPatmir '2012-11-01','2012-11-30'      
CREATE PROCEDURE [dbo].[pAhRptTotalCtasPatmir] (@FecIni SMALLDATETIME, @FecFin SMALLDATETIME)                    
    AS           
...
--select * from #clientesT      
      
SELECT DISTINCT c.codoficina, c.Sucursal,      
       SUM(c.monapertura) AS MontoApertura,      
       SUM(c.saldocta)    AS SaldoCuentas,      
       SUM(c.saldocta) - SUM(c.monapertura) AS Incremento,      
       COUNT(c.codcuenta) AS NumCuentas,      
       (Select count(*)      
          From #clientesT c2      
         Where c.codoficina  = c2.codoficina      
           And Tipo = 1) AS ALAVISTA,      
       (Select count(*)      
          From #clientesT c2      
         Where c.codoficina  = c2.codoficina      
           And Tipo = 2) AS DPF,      
       ...
       (Select count(*)       
          From #clientesT c2      
         Where c.codoficina  = c2.codoficina      
           And c2.Patmir = 'SI') AS TotalCtesPatmir,      
       (Select count(*)       
          From #clientesT c2      
         Where c.codoficina  = c2.codoficina      
           And c2.Patmir <> 'SI') AS TotalCtesNoPatmir,      
       (Select count(*)       
          From #clientesT c2      
         Where c.codoficina  = c2.codoficina      
           And c2.AperturadoMayor500='SI') AS CtasMtoAperMayor500,      
       (Select count(*)       
          From #clientesT c2      
      Where c.codoficina  = c2.codoficina      
           And c2.AperturadoMayor500='SI') AS CtasSaldoCierreMayor500,      
       (Select count(*)       
          From #clientesT c2      
         Where c.codoficina  = c2.codoficina      
           And c2.saldocta>='500' and c2.Patmir = 'SI') AS CtesPatmirMay500,      
       @FecIni FecInicial, @FecFin FechaFinal      
  FROM #clientesT c      
--  where c.codoficina = '2'      
