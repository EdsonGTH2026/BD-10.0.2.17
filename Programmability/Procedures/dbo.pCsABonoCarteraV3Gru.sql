...
GO
--drop procedure pCsABonoCarteraV2Gru
CREATE procedure [dbo].[pCsABonoCarteraV3Gru]	@fecha SMALLDATETIME
as
...
from (
	SELECT c.codasesor
	,sum(case when c.codproducto in('166','123','156','164') then t.montointerestran else 0 end) montointeres
	,sum(case when c.codproducto not in('166','123','156','164') then t.montointerestran else 0 end) montointeres2
...
,case when p.ncliactu-p.ncliante-p.nreasignado<0 then 0 else p.ncliactu-p.ncliante-p.nreasignado end NetoClientesNuevos
...
LEFT OUTER JOIN tCsCaFactoresCalcBono f1 ON (case when p.ncliactu-p.ncliante-p.nreasignado<0 then 0 else p.ncliactu-p.ncliante-p.nreasignado end) between f1.MtoMin and f1.MtoMax and f1.tipo = 26
...
where fecha=@fecha and tipocalculo=3

insert into tCsRptBonoCarteraV2
select b.Fecha,3 Tipo,b.codoficina,b.nomoficina,b.region,b.codasesor,b.nomasesor,b.puesto,b.estadoasesor,e.Ingreso,e.CodEmpleado
