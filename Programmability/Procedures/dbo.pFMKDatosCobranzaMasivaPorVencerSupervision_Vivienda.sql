...
GO
CREATE procedure [dbo].[pFMKDatosCobranzaMasivaPorVencerSupervision_Vivienda] @fecha smalldatetime, @ControlInterno varchar(20), @DiasPorVencer integer, @CodUsuarioCobranza varchar(15)
as
/*
declare @fecha smalldatetime
declare @ControlInterno varchar(20)
declare @DiasPorVencer integer
declare @CodUsuarioCobranza varchar(20)
set @fecha='20141007'
set @ControlInterno = ''
set @DiasPorVencer = 2
set @CodUsuarioCobranza = 'GCF2303811'
*/
...
inner join tCsPadronPlanCuotas p with(nolock) on c.fecha=p.fecha and c.codprestamo = p.codprestamo and c.codusuario = p.codusuario
left outer join tsgusuarios u with(nolock) on u.codusuario=c.codasesor 
where c.fecha= @fecha 
--and c.nrodiasatraso = 0
and p.EstadoCuota = 'VIGENTE' 
--and p.FechaVencimiento <= dateadd(day,@DiasPorVencer, c.fecha) --'20141006'
--and p.FechaPagoConcepto is null
and p.codconcepto = 'CAPI'
and c.codprestamo in (select codprestamo from [10.0.2.14].finmas.dbo.tcaprestamovivienda 
                      where  
                     (fecsup1 >= @fecha and fecsup1 <= dateadd(day,2, @fecha)) 
                      or (fecsup2 >= @fecha and fecsup2 <= dateadd(day,2, @fecha)) 
                      )
