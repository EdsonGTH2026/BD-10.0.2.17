...
--Exec pCsEstadoCuentaCACronograma '010-117-06-00-00011', '20110817'
CREATE Procedure [dbo].[pCsEstadoCuentaCACronograma]
@CodPrestamo	Varchar(25),
@Corte			SmallDateTime
As
--Set @CodPrestamo	= @CodPrestamo
--Set @Corte			= '20110718'
...
If (Select Count(*) From tCsEstadoCuentaCA Where CodPrestamo = @CodPrestamo And Corte = @Corte And Validacion = 1) = 0
Begin 
	...
	Delete From tCsEstadoCuentaCA Where CodPrestamo = @CodPrestamo And Corte = @Corte
	
	Insert Into tCsEstadoCuentaCA (Corte, CodPrestamo, SecCuota, CodConcepto, Devengado, Pago, FechaInicio, FechaVencimiento)
						   ...
						   GROUP BY CodPrestamo, CodConcepto, SecCuota, FechaVencimiento, FechaInicio) AS Cronograma_1 LEFT OUTER JOIN
							(SELECT     CodPrestamo, CodConcepto, SecCuota, SUM(Pago) AS Pago From 
							  (SELECT     CodPrestamo, CodConcepto, SecCuota, SUM(MontoPagado) AS Pago
								...
								WHERE      (Fecha <= @Corte) AND (Extornado = 0) AND (CodPrestamo = @CodPrestamo)
								...
								SELECT     tCsOpRecuperablesDet.CodPrestamo, tCsOpRecuperablesDet.CodConcepto, tCsOpRecuperablesDet.SecCuota, SUM(tCsOpRecuperablesDet.MontoOp) AS Pago
									...
									WHERE     (tCsOpRecuperablesDet.Fecha <= @Corte) AND (tCsOpRecuperablesDet.CodPrestamo = @CodPrestamo) AND (tCsOpRecuperables.TipoOp = '002')
	...
	Set @Cuotas = (Select Max(Seccuota) From tCsEstadoCuentaCA Where CodPrestamo = @CodPrestamo And Corte = @Corte )
	
	/*
	Select * from tCsEstadoCuentaCA 
	Where 	tCsEstadoCuentaCA.CodPrestamo = @CodPrestamo And Corte = @Corte
	Order BY seccuota
	--*/
	
	If (SELECT     Count(*)
			FROM         tCsCartera
			WHERE     (Fecha = @Corte) AND (CodPrestamo = @CodPrestamo) And NroCuotasPagadas < CuotaActual) = 1
	Begin
		Print 'Se actualiza a cero'
		
		UPDATE	tCsEstadoCuentaCA
		Set		Devengado = 0
		Where	Seccuota > (Select CuotaActual from tCsCartera Where Fecha = @Corte And CodPrestamo = @CodPrestamo) 
		And		CodConcepto not in ('CAPI')
		And		tCsEstadoCuentaCA.CodPrestamo = @CodPrestamo And Corte = @Corte
	End
	
	--Se pone en cero cuotas futuras.
	UPDATE	tCsEstadoCuentaCA
	Set		Devengado = 0
	Where	Seccuota IN (Select CuotaActual from tCsCartera Where Fecha = @Corte And CodPrestamo = @CodPrestamo) 
	And		CodConcepto IN ('MORA', 'IVACM', 'INPE', 'IVAMO')
	And		tCsEstadoCuentaCA.CodPrestamo = @CodPrestamo And Corte = @Corte And FechaVencimiento > Corte
	
	UPDATE	tCsEstadoCuentaCA
	Set		Devengado = 0
	Where	Seccuota IN (Select CuotaActual from tCsCartera Where Fecha = @Corte And CodPrestamo = @CodPrestamo) 
	And		CodConcepto IN ('INPE', 'IVAMO')
	And		tCsEstadoCuentaCA.CodPrestamo = @CodPrestamo And Corte = @Corte And SecCuota = @Cuotas
	...
	FROM         (SELECT     Devengado.Fecha, Devengado.CodPrestamo, Devengado.InteresDevengado, Devengado.SaldoInteres, Devengado.MoratorioDevengado, 
						   ...
						   FROM          (SELECT     Fecha, CodPrestamo, SUM(InteresDevengado) AS InteresDevengado, SUM(SaldoInteres) AS SaldoInteres, SUM(MoratorioDevengado) 
												   ...
												   FROM          tCsCarteraDet
												   WHERE      (CodPrestamo = @CodPrestamo) AND (Fecha <= @Corte)
													  ...
													  (SELECT     PlanCuotas_1.CodPrestamo, PlanCuotas_1.SecCuota, Pagos.FechaPago, Pagos.Estado, 
														...
														FROM          (SELECT     CodPrestamo, MAX(UltimoPago) AS UltimoPago, MAX(NroCuotas) AS NroCuotas
																		   FROM          (SELECT     tCsCartera.CodPrestamo, MAX(tCsPagoDet.Fecha) AS UltimoPago, MAX(tCsCartera.NroCuotas) AS NroCuotas
																								   ...
																								   WHERE      (tCsCartera.CodPrestamo = @CodPrestamo) AND (tCsCartera.Fecha = @Corte) AND (tCsPagoDet.Extornado = 0)
																								   GROUP BY tCsCartera.CodPrestamo
																								   UNION
																								   SELECT     CodPrestamo, FechaDesembolso, NroCuotas
																								   FROM         tCsCartera AS tCsCartera_2
																								   WHERE     (CodPrestamo = @CodPrestamo) AND (Fecha = @Corte)) AS Datos
																		   GROUP BY CodPrestamo) AS UltimoPago RIGHT OUTER JOIN
																				   (SELECT DISTINCT CodPrestamo, SecCuota, FechaInicio, FechaVencimiento
																					 ...
																					 WHERE      (CodPrestamo = @CodPrestamo)) AS PlanCuotas_1 ON 
																				   ...
																				   (SELECT     Pagos_1.CodPrestamo, Pagos_1.SecCuota, Pagos_1.FechaPago, CASE WHEN Vigente.Cuota IS NULL 
																					 ...
																					 FROM          (SELECT     CodPrestamo, SecCuota, MAX(FechaPago) AS FechaPago
																											 FROM          (SELECT     CodPrestamo, SecCuota, MAX(Fecha) AS FechaPago
																																	 FROM          tCsPagoDet AS tCsPagoDet_1
																																	 WHERE      (CodPrestamo = @CodPrestamo) AND (Fecha <= @Corte) AND (Extornado = 0)
																																	 GROUP BY CodPrestamo, SecCuota
																																	 UNION
																																	 SELECT     CodPrestamo, 1 AS Cuota, FechaDesembolso
																																	 FROM         tCsCartera
																																	 WHERE     (CodPrestamo = @CodPrestamo) AND (Fecha = @Corte)) AS Datos
																											 GROUP BY CodPrestamo, SecCuota) AS Pagos_1 LEFT OUTER JOIN
																												(SELECT     CodPrestamo, MAX(Cuota) AS Cuota
																												  FROM          (SELECT     CodPrestamo, MAX(SecCuota) AS Cuota
																																		  ...
																																		  UNION
																																		  SELECT     @CodPrestamo AS CodPrestamo, 1 AS Cuota) AS Datos
	...
	Begin
		...
		SELECT  CodPrestamo = @CodPrestamo, Seccuota = @Cuotas + 1,		INTE = 0,	INPE = SUM(Pago), Estado = 'Z', FechaPago = Null
		FROM         (SELECT     SUM(Pago) * - 1 AS Pago
							   FROM          tCsEstadoCuentaCA
							   WHERE      (CodPrestamo = @CodPrestamo) AND (Corte = @Corte) AND (CodConcepto = 'INPE') AND (SecCuota <> @Cuotas)
							   UNION ALL
	...
	FROM         tCsEstadoCuentaCA INNER JOIN
	...
	Set @IVAIT = dbo.fduPIVAPrestamo(1, @CodPrestamo, @Corte)
	Set @IVAMO = dbo.fduPIVAPrestamo(2, @CodPrestamo, @Corte)                
	...
	FROM    tCsEstadoCuentaCA INNER JOIN
			...
			tCsEstadoCuentaCA.SecCuota = Devengado.SecCuota
	WHERE   (tCsEstadoCuentaCA.CodConcepto = 'INTE') And tCsEstadoCuentaCA.CodPrestamo = @CodPrestamo And Corte = @Corte
	...
	FROM	tCsEstadoCuentaCA INNER JOIN
			(Select * from #Resumen Where Estado IN('V', 'Z')) AS Devengado ON tCsEstadoCuentaCA.CodPrestamo = Devengado.CodPrestamo AND 
			 tCsEstadoCuentaCA.SecCuota = Devengado.SecCuota
	WHERE	(tCsEstadoCuentaCA.CodConcepto = 'IVAIT') And tCsEstadoCuentaCA.CodPrestamo = @CodPrestamo And Corte = @Corte
	...
	FROM    tCsEstadoCuentaCA INNER JOIN
						  ...
						  tCsEstadoCuentaCA.SecCuota = Devengado.Seccuota
	WHERE     (tCsEstadoCuentaCA.CodConcepto = 'INPE') AND (tCsEstadoCuentaCA.CodPrestamo = @CodPrestamo) AND (tCsEstadoCuentaCA.Corte = @Corte)
	...
	FROM    tCsEstadoCuentaCA INNER JOIN
						  ...
						  tCsEstadoCuentaCA.SecCuota = Devengado.Seccuota
	WHERE     (tCsEstadoCuentaCA.CodConcepto = 'IVAMO') AND (tCsEstadoCuentaCA.CodPrestamo = @CodPrestamo) AND (tCsEstadoCuentaCA.Corte = @Corte)
	...
	UPDATE tCsEstadoCuentaCA
	Set FechaPago = Null
	Where tCsEstadoCuentaCA.CodPrestamo = @CodPrestamo And Corte = @Corte and Pago < Devengado Or Pago = 0
	...
	Begin
		...
		Begin
			...
			WHERE     (CodPrestamo = @CodPrestamo) AND (Corte = @Corte) AND (CodConcepto IN ('IVAIT', 'IVAMO', 'IVACM')) AND (Devengado - Pago > 0)
		End
		If 	@IVAIT - @IVAMO > 0
		Begin
			Update tCsEstadoCuentaCA
		...
		Update tCsEstadoCuentaCA
		Set Devengado = Pago 
		WHERE     (CodPrestamo = @CodPrestamo) AND (Corte = @Corte) AND (CodConcepto IN ('IVAIT', 'IVAMO', 'IVACM')) AND Estado = 'PAGADO' AND (ABS(Round(Devengado - Pago, 2)) > 0) AND (ABS(Round(Devengado - Pago, 2)) <= 0.01)
	...
	If (Select Count(*) From (Select CodPrestamo, 
		...
		FROM         tCsCartera
		Where CodPrestamo = @CodPrestamo And Fecha = @Corte) as Datos) = 1
	Begin
		Update tCsEstadoCuentaCA
		Set Validacion = 1
		Where CodPrestamo = @CodPrestamo And Corte = @Corte
	...
	Drop Table #Resumen
End 
--Select * From tCsEstadoCuentaCA Where CodPrestamo = @CodPrestamo And Corte = @Corte order by seccuota             
GO