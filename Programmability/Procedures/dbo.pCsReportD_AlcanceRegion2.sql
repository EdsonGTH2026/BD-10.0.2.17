...
CREATE Procedure [dbo].[pCsReportD_AlcanceRegion2] @fecha smalldatetime        
as        
BEGIN    
...
SELECT * FROM FNMGCONSOLIDADO.DBO.TmpROAlcanceRegion WITH(NOLOCK)
WHERE FECHA=@fecha
...
--left outer join tcspadroncarteradet pd  with(nolock) on  cd.codprestamo=pd.codprestamo and cd.codusuario=pd.codusuario      
...
--where  nrodiasatraso<=30 and C.fecha in(@fecante,@fecha)      
-- and cartera='ACTIVA' and c.codoficina not in('97','230','231','999')      
