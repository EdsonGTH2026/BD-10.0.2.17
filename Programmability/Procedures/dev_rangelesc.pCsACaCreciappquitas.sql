...
CREATE procedure [dev_rangelesc].[pCsACaCreciappquitas] @codoficina varchar(4)                
as                
set nocount on                   
--begin tran                
...
---modificacion dias de atraso               
              
select p.codprestamo, cd.fechacambioasesor               
into #ptmosVencidosfech                
from  #ptmosVencidos p                  
inner join [10.0.2.14].[finmas].[dbo].[tcaprestamos] cd on cd.codprestamo=p.codprestamo              
where cd.fechacambioasesor is not null and  cd.fechacambioasesor>=@fecini              
              
select p.codprestamo              
into #ptmosVencidosnew                 
from #ptmosVencidosfech  p              
inner join tcscartera cd with(nolock) on cd.codprestamo=p.codprestamo and cd.fecha=p.fechacambioasesor              
and cd.nrodiasatraso>=30              
...
select p.codprestamo,cd.saldocapital                
into #CAiniSaldonew                 
from #CAiniSaldo  p              
inner join tcscarteradet cd with(nolock) on cd.codprestamo=p.codprestamo and cd.fecha=p.fechacambioasesor          
        
        
update #CAini        
set saldocapital=cd.saldocapital        
from #CAini f        
inner join #CAiniSaldonew cd on f.codprestamo=cd.codprestamo        
...
from (                      
 select i.promotor                      
 ,case when i.codpromotor<>p.ultimoasesor --@fecha                       
    then 'QUITADO'                      
    else 'CANCELADO' end TIPO                      
 ...
 inner join #CAiniSaldonew  w with(nolock) on w.codprestamo=i.codprestamo                          
 --left outer join tcscartera c with(nolock) on c.codprestamo=p.codprestamo and c.fecha=@fecha                      
 where i.codprestamo not in(select codprestamo from #ptmosVencidosnew with(nolock)) and f.codprestamo is null                       
 --and i.promotor='CASTILLO GUZMAN MARIA REYNALDA'                      
 group by i.promotor,case when i.codpromotor<>p.ultimoasesor --@fecha                       
    then'QUITADO'                       
    else 'CANCELADO' end                      
) x                      
group by promotor                      
                
create table #ptmosliq (codprestamo varchar(25),cancelacion datetime,desembolso datetime,codasesor varchar(35))            
insert into #ptmosliq            
--select distinct codprestamo from tcscartera with(nolock)            
select distinct codprestamo,cancelacion,desembolso,UltimoAsesor from tcspadroncarteradet with(nolock)            
where cancelacion>=@fecini and cancelacion<=@fecha            
and carteraactual='ACTIVA' and codoficina not in('97','230','231','999')            
and codprestamo not in (select codprestamo from tCsCarteraAlta with(nolock))            
--and codprestamo not in(select codprestamo from #RE)             
            
insert into #CAFin            
select codasesor,case when (e.codusuario is null or e.codpuesto<>66) then 'HUERFANO' else pr.nombrecompleto end promotor,codprestamo,0,desembolso            
from #ptmosliq c            
inner join tcspadronclientes pr with(nolock) on pr.codusuario=c.codasesor              
left outer join tcsempleadosfecha e with(nolock) on e.codusuario=c.codasesor and e.fecha=@fecha            
            
create table #ptmoscast (codprestamo varchar(25),cancelacion datetime,desembolso datetime,codasesor varchar(35))            
...
set saldocapital=cd.saldocapital        
from #CAFin f        
inner join #CAFinSaldonew cd on f.codprestamo=cd.codprestamo        
...
,case when f.fechadesembolso>=@fecini and f.fechadesembolso<=@fecha then                  
 ...
 from #CAfin f left outer join #CAini i on i.promotor=f.promotor and i.codprestamo=f.codprestamo                      
    ...
    inner join #CAFinSaldonew w with(nolock) on w.codprestamo=f.codprestamo                          
...
drop table #CAFinSaldonew             
drop table #CAFinSaldo             
drop table #CAiniSaldonew        
drop table #CAiniSaldo                    
...
drop table #cua                
drop table #ptmosVencidosfech               
drop table #ptmosVencidosnew                
