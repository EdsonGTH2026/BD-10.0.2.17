...
--GO
--/****** Object:  StoredProcedure [dbo].[pCsCalculoRFC]    Script Date: 16/05/2017 06:15:19 pm ******/
--SET ANSI_NULLS ON
--GO
--SET QUOTED_IDENTIFIER ON
--GO
----Drop Procedure pCsCalculoRFC

CREATE FUNCTION [dbo].[fduCalculoRFC] (@NOMBRES_AUX VARCHAR(100),@APATERNO_AUX VARCHAR(100),@AMATERNO_AUX VARCHAR(100),@FECHANACIMIENTO SmallDateTime)
RETURNS varchar(19)
AS
begin
--DECLARACION DE VARIABLES
declare @RFC_OUT Varchar(16)
...
BEGIN
    ...
    IF @NOMBRE1 IN ('JOSE','MARIA','MA.','DE','LA','LAS','MC','VON','DEL','LOS','Y','MAC','VAN') AND @NOMBRE2 <> ''
    BEGIN
        SET @NOMBRES = @NOMBRE2
    END 
    ELSE
...
BEGIN
    ...
    CASE
        ...
        WHEN SUBSTRING (@T_NOMTOT , @VARLOOPS , 1) >= '0' AND SUBSTRING (@T_NOMTOT , @VARLOOPS , 1) <= '9' 
        THEN CONVERT(VARCHAR,CONVERT(INT, SUBSTRING (@T_NOMTOT , @VARLOOPS , 1)) , 2)
        WHEN SUBSTRING (@T_NOMTOT , @VARLOOPS , 1) IN ('&','Ñ') 
        THEN '10'
        ELSE '00'
    END
...
BEGIN
    ...
    CASE
        ...
        WHEN SUBSTRING (@RFC , @VARLOOPS , 1) >= '0' AND SUBSTRING (@RFC , @VARLOOPS , 1) <= '9'
...
IF substring(@RFC_OUT,1, 4) in (	'BUEI', 'CACA', 'CAGA', 'CAKA', 'COGE', 'COJE', 'COJO', 'FETO', 'JOTO', 'KACO', 'KAGO', 'KOJO', 'KULO', 'MAMO', 'MEAS', 'MION', 'MULA', 'PEDO', 'PUTA', 'QULO', 'RUIN',
			'BUEY', 'CACO', 'CAGO', 'CAKO', 'COJA', 'COJI', 'CULO', 'GUEY', 'KACA', 'KAGA', 'KOGE', 'KAKA', 'MAME', 'MEAR', 'MEON', 'MOCO', 'PEDA', 'PENE', 'PUTO', 'RATA')
BEGIN
	Set @RFC_OUT = substring(@RFC_OUT,1, 3) + 'X' + substring(@RFC_OUT,5, 50)
END

--Print 'RFC: ' +  Isnull(@RFC_Out , 'NULO')
return (@RFC_OUT)
end

GO

GRANT EXECUTE ON [dbo].[fduCalculoRFC] TO [rie_jaguilar]
GO

GRANT EXECUTE ON [dbo].[fduCalculoRFC] TO [rie_sbravoa]
GO

GRANT EXECUTE ON [dbo].[fduCalculoRFC] TO [rie_ldomingueze]
GO

GRANT EXECUTE ON [dbo].[fduCalculoRFC] TO [rie_jalvarezc]
GO

GRANT EXECUTE ON [dbo].[fduCalculoRFC] TO [rie_blozanob]
GO