...
SET ANSI_NULLS OFF
GO
--drop function dbo.fduTablaSemanaPeriodosFC
CREATE FUNCTION [dbo].[fduTablaSemanaPeriodosFC] (@Tperiodo varchar(200))  
RETURNS @se TABLE (
      nrosemana int,
...
select @fc=fechaconsolidacion from vcsfechaconsolidacion

delete from @se
where nrosemana > (
select max(nrosemana) from @se
where fechaini<=@fc)

declare @f smalldatetime
select @f=fechafin from @se where nrosemana=(select max(nrosemana) from @se where fechaini<=@fc)
...
GRANT SELECT ON [dbo].[fduTablaSemanaPeriodosFC] TO [marista]
GO

GRANT SELECT ON [dbo].[fduTablaSemanaPeriodosFC] TO [jarriagaa]
GO