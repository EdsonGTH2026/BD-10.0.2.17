...
GO
CREATE VIEW [dbo].[vCsArreglarCodigos6]
AS
SELECT DISTINCT Datos.CC, Datos.CodUsuario
FROM         (SELECT     CC
                       FROM          (SELECT     tCsPadronCarteraDet.CodPrestamo, Datos.CC, tCsPadronCarteraDet.CodUsuario, CHARINDEX(LEFT(RIGHT(RTRIM(Datos.CC), 15), 6), 
                       ...
                       HAVING      (COUNT(*) = 1)) corte INNER JOIN
                          (SELECT     tCsPadronCarteraDet.CodPrestamo, Datos.CC, tCsPadronCarteraDet.CodUsuario, CHARINDEX(LEFT(RIGHT(RTRIM(Datos.CC), 15), 6), 
                                                   tCsPadronCarteraDet.CodUsuario, 1) AS Valida
                            FROM          (SELECT DISTINCT 
                                                                           tCsCarteraDet.CodUsuario AS CC, tCsCarteraDet.CodPrestamo AS PC, tCsPadronCarteraDet.CodUsuario AS CP, 
                                                                           tCsPadronCarteraDet.CodPrestamo AS PP
