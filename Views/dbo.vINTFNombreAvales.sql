...
CREATE VIEW [dbo].[vINTFNombreAvales]
AS
...
FROM         (SELECT     CodUsuario, CASE c WHEN 10 THEN usrfc ELSE usrfcbd END AS UsRFC
                      ...
                      dbo.tINTFPeriodo INNER JOIN
                          (SELECT     tCsCartera.Fecha, tCsMesGarantias.Codigo AS CodPrestamo, tCsPadronClientes.CodUsuario, tCsMesGarantias.EstGarantia
                            FROM          tCsMesGarantias with(nolock) INNER JOIN
                                                   tCsClientes with(nolock) ON tCsMesGarantias.DocPropiedad = tCsClientes.CodOrigen INNER JOIN
                                                   tCsPadronClientes with(nolock) ON tCsClientes.CodUsuario = tCsPadronClientes.CodOriginal INNER JOIN
                                                   tCsCartera with(nolock) ON dbo.fduFechaATexto(tCsMesGarantias.Fecha, 'AAAAMM') = dbo.fduFechaATexto(tCsCartera.Fecha, 'AAAAMM') AND 
                                                   tCsMesGarantias.Codigo = tCsCartera.CodPrestamo
                            WHERE      (tCsCartera.NroDiasAtraso >= 90)) tCsCarteraDet ON dbo.tINTFPeriodo.Corte = tCsCarteraDet.Fecha ON 
                      dbo.tCsPadronClientes.CodUsuario = tCsCarteraDet.CodUsuario
WHERE     (dbo.tINTFPeriodo.Activo = 1)  AND (tCsCarteraDet.EstGarantia NOT IN ('INACTIVO'))
