...
CREATE VIEW [dbo].[vINTFDireccionFamiliar]
AS
SELECT     dbo.vINTFNombre.CodUsuario, RTRIM(LTRIM(REPLACE(dbo.tCsPadronClientes.DireccionDirFamPri, '  ', ' '))) AS Direccion1, 
                      ...
                      '' AS FechaResidencia, '' AS Telefono, '' AS Extencion, '' AS Fax, 'H' AS Tipo, 
                      CASE WHEN tCPLugar.Zona = 'Rural' THEN 'R' WHEN rtrim(ltrim(DireccionDirFamPri)) = 'DOMICILIO CONOCIDO' THEN 'K' ELSE '' END AS Indicador, 
                      dbo.tCsPadronClientes.CodUbiGeoDirFamPri AS CodUbigeo
FROM         dbo.tCPClCiudad RIGHT OUTER JOIN
                      ...
                      dbo.tClUbigeo.CodUbiGeo = dbo.tCsPadronClientes.CodUbiGeoDirFamPri
