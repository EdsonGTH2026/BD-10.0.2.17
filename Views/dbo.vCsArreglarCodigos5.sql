...
GO
CREATE VIEW [dbo].[vCsArreglarCodigos5]
AS
SELECT DISTINCT Datos.CC, Datos.CodUsuario
FROM         (SELECT     CC
                       FROM          (SELECT     tCsPadronCarteraDet.CodPrestamo, Datos.CC, tCsPadronCarteraDet.CodUsuario, CHARINDEX(LEFT(RIGHT(RTRIM(Datos.CC), 7), 6), 
                       ...
                       HAVING      (COUNT(*) = 1)) corte INNER JOIN
                          (SELECT     tCsPadronCarteraDet.CodPrestamo, Datos.CC, tCsPadronCarteraDet.CodUsuario, CHARINDEX(LEFT(RIGHT(RTRIM(Datos.CC), 7), 6), 
                                                   tCsPadronCarteraDet.CodUsuario, 1) AS Valida
                            FROM          (SELECT DISTINCT 
                                                                           tCsCarteraDet.CodUsuario AS CC, tCsCarteraDet.CodPrestamo AS PC, tCsPadronCarteraDet.CodUsuario AS CP, 
                                                                           tCsPadronCarteraDet.CodPrestamo AS PP
...
Begin DesignProperties = 
   ...
   End
End
', 'SCHEMA', N'dbo', 'VIEW', N'vCsArreglarCodigos5'
GO

EXEC sys.sp_addextendedproperty N'MS_DiagramPaneCount', 1, 'SCHEMA', N'dbo', 'VIEW', N'vCsArreglarCodigos5'
GO