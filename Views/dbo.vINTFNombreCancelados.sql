...
CREATE VIEW [dbo].[vINTFNombreCancelados]
AS
SELECT     	'Cancelados' AS Tipo, tCsCarteraDet.Fecha, tCsCarteraDet.CodPrestamo, tCsCarteraDet.CodUsuario, 
...
FROM         (SELECT     CodUsuario, CASE c WHEN 10 THEN usrfc ELSE usrfcbd END AS UsRFC
                      ...
                      dbo.tINTFPeriodo INNER JOIN
                          (SELECT     CAST(dbo.fduFechaATexto(DATEADD([Month], 1, CAST(dbo.fduFechaATexto(Cancelacion, 'AAAAMM') + '01' AS SmallDateTime)), 'AAAAMM') 
                                                   + '01' AS SmallDateTime) - 1 AS Fecha, Cancelacion, CodPrestamo, CodUsuario
                            FROM          tCsPadronCarteraDet with(nolock)
                            WHERE      (EstadoCalculado = 'CANCELADO')) tCsCarteraDet ON dbo.tINTFPeriodo.Corte = tCsCarteraDet.Fecha ON 
